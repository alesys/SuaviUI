...
local function GetSettings()
    ...
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.general then
        return SUICore.db.profile.general.quickSalvage
    end
    return nil
end
...
local SalvageButton = CreateFrame("Button", "SuaviUI_QuickSalvageButton", UIParent, table.concat(TEMPLATES, ','))
...
eventFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    elseif event == "MODIFIER_STATE_CHANGED" then
        ...
        elseif GameTooltip:IsShown() and IsModifierActive() then
            ...
            local ownerMouseOver = owner and pcall(owner.IsMouseOver, owner)
            if ownerMouseOver then
                if owner.GetSlotAndBagID then
...
---------------------------------------------------------------------------
function _G.SuaviUI_RefreshQuickSalvage()
    if not InCombatLockdown() then
        SalvageButton:UpdateAttributeDriver()
    end
end
...
QuickSalvage.Refresh = _G.SuaviUI_RefreshQuickSalvage
