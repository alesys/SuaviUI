...
function SuaviUI:BackwardsCompat()
    ...
    if self.db and self.db.profile and self.db.profile.suiUnitFrames and self.db.profile.suiUnitFrames.general then
        MigrateMasterTextColors(self.db.profile.suiUnitFrames.general)
    end
    ...
    if SuaviUI_DB and SuaviUI_DB.profiles and SuaviUI_DB.profiles.Default then
        self:DebugPrint("Profiles.Default.imports Exists: " .. tostring(not (not SuaviUI_DB.profiles.Default.imports)))
        ...
        if SuaviUI_DB.profiles.Default.imports and (not self.db.global.imports or next(self.db.global.imports) == nil) then
            self:DebugPrint("Import Data found in profile imports but not global imports.")
            self.db.global.imports = SuaviUI_DB.profiles.Default.imports
        end
    end
end
