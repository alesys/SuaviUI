...
- **Key Documents:**
  ...
  - `RELEASE_PROCESS.md` - Complete automated release workflow (commit, tag, push, ZIP, publish)
  - `LEM_SETTINGS_COMPLETE_REFERENCE.md` - LibEQOLEditMode API reference (1200+ lines)
  - Various technical documentation for castbars, action bars, resource bars, etc.
- **Usage:** Always check these documents for compatibility information and established workflows before suggesting changes.
...
4. **Context Preservation**: When using `replace_string_in_file`, include 3–5 lines of unchanged context around the change (enough to uniquely identify the section, but not excessive).
...
3. **READ COMPREHENSIVELY** - Study the COMPLETE implementation, not just snippets:
   ...
   - **Trace data flow** - How does data move from settings to display?
     - Settings `set()` callback → Database write → Method call → Visual update
...
4. **CREATE IMPLEMENTATION MAP** - Before writing code, document:
   ...
   ☐ GetSize() with widthMode support
   ☐ GetPoint() with relativeFrame resolution
   ☐ Apply*Settings() methods (5 different ones)
   ☐ resolveRelativeFrames() helper
   ☐ availableRelativeFrames() dynamic dropdown
   ☐ Proper layoutName parameter passing
   ☐ LEM.internal:RefreshSettingValues() calls
   ☐ Edit mode drag disabling
...
3. Map event flow: Registration → Trigger → Handler → Update
...
Core Methods Needed:
☐ ApplyLayout - Complete implementation traced
☐ GetSize - Including all width mode logic
☐ GetPoint - Including frame resolution
☐ Apply*Settings - All 5+ specialized methods
☐ Helper utilities - rounded, clamp, resolve*, etc.

Database Integration:
☐ Read path: SenseiDB[dbName][layout].field
☐ Write pattern: DB[dbName][layout] = DB... or CopyTable(defaults)
☐ Default handling: Check nil, fallback to defaults
☐ Layout creation: CopyTable when layout doesn't exist

Settings Panel:
☐ Correct field names (minValue not min)
☐ Proper data types (generator for functions, values for tables)
☐ Dynamic dropdowns use generator
☐ Static dropdowns use values array
☐ All settings call ApplyLayout(layoutName)
☐ Special settings call RefreshSettingValues

Testing Plan:
☐ Change X position slider → Bar moves horizontally
☐ Change Y position slider → Bar moves vertically
☐ Drag bar in Edit Mode → Sliders update
☐ Change Relative Frame → Bar anchors correctly
☐ Switch layout → Settings persist per layout
☐ Create new layout → Copies defaults properly
```
...
❌ Implementing settings before methods they call
❌ Guessing at field names or method signatures
❌ Skipping helper functions ("we don't need that")
❌ Writing all settings at once without testing
❌ Assuming "it's similar enough" to skip reading
❌ Not tracing the complete data flow
**When told to match another addon's implementation (e.g., "use Sensei implementation"):**
1. **STOP** - Do not write any code yet
2. **LOCATE** - Find the actual reference files in the workspace (e.g., check if SenseiClassResourceBar exists in AddOns folder)
...
1. ❌ **Assuming field names** - `min` vs `minValue`, `options` vs `values`
2. ❌ **Mixing APIs** - Using function where table expected
3. ❌ **Copy-paste errors** - Missing closing braces
4. ❌ **Unrealistic values** - Width 0-1 when should be 1-500
5. ❌ **Skipping validation** - Not checking syntax before claiming "done"
6. ❌ **Rushing** - Writing 300 lines without incremental testing
...
**Completed:**
- ✅ Mixin hierarchy created (BarMixin → PowerBarMixin → CDMAlignedMixin → SpecificBars)
- ✅ 14 modular files replacing monolithic implementation
- ✅ LibEQOL integrated for Edit Mode support
- ✅ Bars initialize and can display
...
- ✅ **Phase 2: Core Layout Methods COMPLETE**
...
- ✅ **Phase 4: Parameter Passing COMPLETE**
  - ALL set() callbacks updated to pass layoutName to ApplyLayout()
  - 20+ settings now correctly pass layout parameter

**PENDING TESTING:**
- ⏸️ Settings panel changes → Bar updates (should work now)
- ⏸️ Dragging bars in Edit Mode → Position sliders update (LibEQOL handles this)
- ⏸️ Relative Frame dropdown → Should now show 15+ options
- ⏸️ Cyclic frame reference detection works correctly

**TODO (Phase 3 Enhancement - Optional):**
- 🔲 ApplyForegroundSettings() - dedicated method (currently in UpdateAppearance)
- 🔲 ApplyBackgroundSettings() - dedicated method (currently in UpdateAppearance)
- 🔲 ApplyFontSettings() - dedicated method (currently in UpdateAppearance)
- 🔲 ApplyFillDirectionSettings() - dedicated method (currently in UpdateAppearance)
- 🔲 Remaining 10+ settings from Sensei (optional expansion)
...
**Phase 5: Validation**
- [ ] Drag bar → position sliders update
- [ ] Change X/Y sliders → bar moves immediately
- [ ] Change Relative Frame → bar anchors to new frame
- [ ] Change texture → bar appearance updates
- [ ] Change width/height → bar resizes
- [ ] Switch layouts → settings persist correctly
