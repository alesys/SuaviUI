...
local function GetLockoutCacheTTL()
    local db = SUICore.db and SUICore.db.profile and SUICore.db.profile.datatext
...
function Datatexts:Register(id, datatextDef)
    if self.registry[id] then
        print("|cffff0000SuaviUI:|r Datatext '" .. id .. "' is already registered!")
        return false
    end
    
    if not datatextDef.OnEnable or type(datatextDef.OnEnable) ~= "function" then
        print("|cffff0000SuaviUI:|r Datatext '" .. id .. "' missing OnEnable function!")
        return false
    end
...
function Datatexts:AttachToSlot(slotFrame, datatextID, settings)
    if not slotFrame then
        print("|cffff0000SuaviUI:|r Invalid slot frame provided")
        return false
    end
    ...
    if not success then
        print("|cffff0000SuaviUI:|r Failed to enable datatext '" .. datatextID .. "': " .. tostring(instance))
        return false
    end
...
Datatexts:Register("time", {
    ...
    OnEnable = function(slotFrame, settings)
        ...
        local function Update()
            ...
            local dtSettings = SUICore.db and SUICore.db.profile and SUICore.db.profile.datatext
...
-- Use shared dungeon data from sui_dungeon_data.lua
local function GetShortDungeonName(mapID)
    if _G.SUI_DungeonData then
        return _G.SUI_DungeonData.GetShortName(mapID)
    end
                            ...
                            if specName then
                                ...
                                root:CreateButton(iconText .. " " .. specName .. (isActive and activeMarker or ""), function()
                                    if InCombatLockdown() then
                                        print("|cffFF6B6BSuaviUI:|r Cannot change specialization in combat")
                                        return
                                    end
