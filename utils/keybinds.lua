...
local function IsAnyKeybindFeatureEnabled()
    ...
    if not SUICore or not SUICore.db or not SUICore.db.profile then return false end

    local viewers = SUICore.db.profile.viewers
...
SUI.FormatKeybind = FormatKeybind
...
local function ApplyKeybindToIcon(icon, viewerName)
    local SUICore = _G.SuaviUI and _G.SuaviUI.SUICore
    if not SUICore or not SUICore.db or not SUICore.db.profile then return end
...
_G.SuaviUI_UpdateViewerKeybinds = function(viewerName)
    -- PERFORMANCE: Skip if no keybind features are enabled
    if not IsAnyKeybindFeatureEnabled() then return end
    UpdateViewerKeybinds(viewerName)
end
...
SLASH_SUIKEYBINDS1 = "/suikeybinds"
SlashCmdList["SUIKEYBINDS"] = function(msg)
    if msg == "debug" then
        RebuildCache()
        DebugPrintCache()
...
_G.SuaviUI_RefreshKeybinds = UpdateAllKeybinds
_G.SuaviUI_RefreshRotationHelper = RefreshRotationHelper
