...
local function GetSettings()
    ...
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.character then
        return SUICore.db.profile.character
    end
...
local function GetEnchantText(unit, slotId)
    ...
    -- OffHand: only enchantable if it's a weapon (dual-wield), not shield/held-in-off-hand
    if slotId == INVSLOT_OFFHAND then
        local invType = select(4, GetItemInfoInstant(itemLink))
        if invType == "INVTYPE_WEAPON" or invType == "INVTYPE_WEAPONOFFHAND" then
            enchantableSlots[INVSLOT_OFFHAND] = true
        end
    end
...
local function IsSkinningHandlingBackground()
    local skinningAPI = _G.SUI_CharacterFrameSkinning
...
---------------------------------------------------------------------------
-- Create custom SuaviUI background
---------------------------------------------------------------------------
local function CreateCustomBackground()
    ...
    if skinningAPI and skinningAPI.IsEnabled and skinningAPI.IsEnabled() then
        ...
        if not customBg then
            customBg = CreateFrame("Frame", "SUI_CharacterFrameBg_CharPane", CharacterFrame, "BackdropTemplate")
...
_G.SuaviUI_RefreshCharacterPanelFonts = RefreshCharacterPanelFonts
...
local function UpdateStatsPanel(panel, unit)
    ...
    if not success then
        print("SuaviUI: Error updating stats panel:", err)
    end
end
...
local function UpdateILvlDisplay()
    if not CharacterFrame or not CharacterFrame._suiILvlDisplay then return end

    local settings = GetSettings()
    if not settings.enabled then return end

    local displayFrame = CharacterFrame._suiILvlDisplay
...
ScheduleUpdate = function()
    ...
    C_Timer.After(0.05, function()
        ...
        -- Update inspect frame if visible (delegated to sui_inspect.lua)
        if ns.SUI.InspectPane and ns.SUI.InspectPane.UpdateInspectFrame then
            ns.SUI.InspectPane.UpdateInspectFrame()
        end
    end)
end
...
local function CreateEquipMgrPopup()
    ...
    local PANEL_WIDTH_EXTENSION = 55
    equipMgrPopup = CreateFrame("Frame", "SuaviUI_EquipMgrPopup", UIParent, "BackdropTemplate")
    ...
    _G.SuaviUI_EquipMgrPopup = equipMgrPopup
...
local function CreateTitlesPopup()
    ...
    -- Position accounts for extended character pane (55px width extension + 10px gap)
    local PANEL_WIDTH_EXTENSION = 55
    titlesPopup = CreateFrame("Frame", "SuaviUI_TitlesPopup", UIParent, "BackdropTemplate")
    ...
    -- Expose globally for skinning module to access
    _G.SuaviUI_TitlesPopup = titlesPopup
...
eventFrame:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" then
        -- Note: Blizzard_InspectUI is now hooked by sui_inspect.lua
...
---------------------------------------------------------------------------
_G.SuaviUI_RefreshCharacterPane = function()
    ScheduleUpdate()
end
...
ns.CharacterPane = SUI.CharacterPane

---------------------------------------------------------------------------
-- Shared exports for sui_inspect.lua
