--[[
    SUI NCDM - New Cooldown Display Manager
...
---------------------------------------------------------------------------
-- HELPER: Square icon overrides from global Cooldown Viewers settings
---------------------------------------------------------------------------
local function GetSquareIconOverrides(viewerKey)
    local profile = SUICore and SUICore.db and SUICore.db.profile
    if not profile then
        return nil
    end

    local enabledKey = "cooldownManager_squareIcons_" .. viewerKey
    if not profile[enabledKey] then
        return nil
    end

    local borderKey = "cooldownManager_squareIconsBorder_" .. viewerKey
    local zoomKey = "cooldownManager_squareIconsZoom_" .. viewerKey

    return {
        aspectRatioCrop = 1.0,
        borderSize = profile[borderKey] ~= nil and profile[borderKey] or 4,
        zoom = profile[zoomKey] ~= nil and profile[zoomKey] or 0,
    }
end
...
combatEndFrame:SetScript("OnEvent", function()
    ...
    C_Timer.After(0.1, function()
        if not InCombatLockdown() and _G.SuaviUI_RefreshNCDM then
...
local function LayoutViewer(viewerName, trackerKey)
    ...
    local squareOverrides = GetSquareIconOverrides(viewerKey)
    local rows = {}
    for i = 1, 3 do
        local rowKey = "row" .. i
        if settings[rowKey] and settings[rowKey].iconCount and settings[rowKey].iconCount > 0 then
            ...
            if squareOverrides then
                rowConfig.aspectRatioCrop = squareOverrides.aspectRatioCrop
                rowConfig.borderSize = squareOverrides.borderSize
                rowConfig.zoom = squareOverrides.zoom
            end

            table.insert(rows, rowConfig)
        end
    end
    ...
    if not viewer.__cdmUpdatePending and not viewer.isEditing then
        ...
        C_Timer.After(0.05, function()
            ...
            if trackerKey == "essential" then
                ...
                if _G.SuaviUI_UpdateLockedCastbarToEssential then
                    _G.SuaviUI_UpdateLockedCastbarToEssential()
                end
            elseif trackerKey == "utility" then
                if _G.SuaviUI_UpdateLockedPowerBarToUtility then
                    _G.SuaviUI_UpdateLockedPowerBarToUtility()
                end
                if _G.SuaviUI_UpdateLockedSecondaryPowerBarToUtility then
                    _G.SuaviUI_UpdateLockedSecondaryPowerBarToUtility()
                end
                if _G.SuaviUI_UpdateLockedCastbarToUtility then
                    _G.SuaviUI_UpdateLockedCastbarToUtility()
                end
            end
            ...
            if _G.SuaviUI_UpdateViewerKeybinds then
                _G.SuaviUI_UpdateViewerKeybinds(viewerName)
            end
        end)
    end
end
    ...
    viewer:HookScript("OnShow", function(self)
        ...
        C_Timer.After(0.02, function()
            if self:IsShown() then
                ...
                if trackerKey == "utility" and _G.SuaviUI_ApplyUtilityAnchor then
                    _G.SuaviUI_ApplyUtilityAnchor()
                end
            end
        end)
...
local function GetCDMFrames()
    ...
    -- SuaviUI power bars - always include in CDM visibility control
    -- (standalone mode only affects positioning, not visibility)
    if SUICore then
        if SUICore.powerBar then
            table.insert(frames, SUICore.powerBar)
        end
        if SUICore.secondaryPowerBar then
            table.insert(frames, SUICore.secondaryPowerBar)
        end
    end
...
local function GetCDMVisibilitySettings()
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.cdmVisibility then
        return SUICore.db.profile.cdmVisibility
    end
    return nil
end
...
local function GetUnitframesVisibilitySettings()
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.unitframesVisibility then
        return SUICore.db.profile.unitframesVisibility
    end
    return nil
end
...
local function GetUnitframeFrames()
    ...
    -- Collect unit frames
    if _G.SuaviUI_UnitFrames then
        for unitKey, frame in pairs(_G.SuaviUI_UnitFrames) do
            if frame then
    ...
    if not (vis and vis.alwaysShowCastbars) then
        if _G.SuaviUI_Castbars then
            for unitKey, castbar in pairs(_G.SuaviUI_Castbars) do
                if castbar then
...
local function UpdateUnitframesVisibility()
    ...
    if _G.SuaviUI_Castbars then
        ...
        if vis and vis.alwaysShowCastbars then
            ...
            if _G.SuaviUI_UnitFrames then
                for _, frame in pairs(_G.SuaviUI_UnitFrames) do
                    if frame then
...
_G.SuaviUI_RefreshCDMVisibility = UpdateCDMVisibility
_G.SuaviUI_RefreshUnitframesVisibility = UpdateUnitframesVisibility
_G.SuaviUI_RefreshCDMMouseover = SetupCDMMouseoverDetector
