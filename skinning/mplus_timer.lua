---------------------------------------------------------------------------
-- SuaviUI Mythic+ Timer Skinning
-- Applies SUI skin colors to the custom M+ timer frame
-- Frame created in utils/SUI_mplus_timer.lua
---------------------------------------------------------------------------
local ADDON_NAME, ns = ...
...
local function ApplyBackdrop(frame, sr, sg, sb, sa, bgr, bgg, bgb, bga, showBorder)
    ...
    if not frame.suiBackdrop then
        ...
        frame.suiBackdrop:SetFrameLevel(math.max(1, frame:GetFrameLevel() - 1))
    ...
    frame.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, borderAlpha)
end
...
---------------------------------------------------------------------------
_G.SuaviUI_ApplyMPlusTimerSkin = ApplyMPlusTimerSkin
_G.SuaviUI_RefreshMPlusTimerColors = RefreshMPlusTimerColors
