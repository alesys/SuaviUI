...
local ADDON_NAME, ns = ...
local SUI = SuaviUI  -- Use global addon table, not ns.Addon (which is SUICore)
...
-- Will be set during initialization when SUICore is available
local SUICore
...
local function GetDB()
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.customTrackers then
        return SUICore.db.profile.customTrackers
    end
    return nil
end

local function GetGlobalDB()
    if SUICore and SUICore.db and SUICore.db.global then
        return SUICore.db.global
    end
    return nil
end
...
local function GetSpellBuffInfo(spellID)
    ...
    local scanner = SUI and SUI.SpellScanner
...
local PROC_GLOW_MASK = "Interface\\AddOns\\SuaviUI\\assets\\iconskin\\ProcGlowMask"
...
local function CreateTrackerIcon(parent)
    ...
    icon:SetScript("OnEnter", function(self)
        ...
        if self.entry then
            ...
            local tooltipSettings = SUI.SUICore and SUI.SUICore.db and SUI.SUICore.db.profile and SUI.SUICore.db.profile.tooltip
...
local function RebuildActiveSet(bar)
    ...
    -- print("|cFF00FF00[SUI Debug]|r RebuildActiveSet: " .. #bar.activeIcons .. " of " .. #(bar.icons or {}) .. " icons active")
end
...
function CustomTrackers:CreateBar(barID, config)
    ...
    local bar = CreateFrame("Frame", "SUI_CustomTracker_" .. barID, UIParent, "BackdropTemplate")
...
---------------------------------------------------------------------------
_G.SuaviUI_RefreshCustomTrackers = function()
    -- Use local CustomTrackers directly since SUICore might not be set yet
...
initFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    if event == "PLAYER_ENTERING_WORLD" then
        -- Set SUICore reference now that it's available
        SUICore = SUI.SUICore
        if SUICore then
            SUICore.CustomTrackers = CustomTrackers
        end
...
local function GetCustomTrackersVisibilitySettings()
    local db = SUI.SUICore and SUI.SUICore.db
...
---------------------------------------------------------------------------
_G.SuaviUI_RefreshCustomTrackersVisibility = UpdateCustomTrackersVisibility
_G.SuaviUI_RefreshCustomTrackersMouseover = SetupCustomTrackersMouseoverDetector

-- Refresh keybind display on all custom tracker icons
_G.SuaviUI_RefreshCustomTrackerKeybinds = function()
