...
local function IsSkinningEnabled()
    local coreRef = _G.SuaviUI and _G.SuaviUI.SUICore
...
local function CreateOrUpdateBackground()
    ...
    if not customBg then
        customBg = CreateFrame("Frame", "SUI_InspectFrameBg_Skin", InspectFrame, "BackdropTemplate")
...
---------------------------------------------------------------------------
-- Main skinning setup
-- Note: OnShow hook is handled by SUI_character.lua which calls SetExtended()
...
local function SetupInspectFrameSkinning()
    ...
    -- Create initial background (will be positioned by SUI_character.lua via SetExtended)
    CreateOrUpdateBackground()
...
_G.SuaviUI_RefreshInspectColors = RefreshInspectFrameColors
