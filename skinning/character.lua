...
SUICore.CharacterSkinning = CharacterSkinning
...
-- Static colors (text only - bg/border from SUI skin system)
local COLORS = {
    text = { 0.9, 0.9, 0.9, 1 },
}
...
---------------------------------------------------------------------------
-- Helper: Get skin colors from SUI system
---------------------------------------------------------------------------
...
local function GetFontPath()
    ...
    local settings = SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.character
...
local function CreateOrUpdateBackground()
    ...
    if not customBg then
        customBg = CreateFrame("Frame", "SUI_CharacterFrameBg_Skin", CharacterFrame, "BackdropTemplate")
...
---------------------------------------------------------------------------
-- API: Set background extended mode (called by SUI_character.lua)
---------------------------------------------------------------------------
...
local function SkinReputationEntry(child)
    ...
    if ReputationBar then
        ...
        if not ReputationBar.suiBackdrop then
            ...
            backdrop:Show()
            ReputationBar.suiBackdrop = backdrop
        end
...
local function SetupCharacterFrameSkinning()
    ...
    -- (SUI_character.lua will extend if character pane customization is enabled)
    if PaperDollFrame then
        PaperDollFrame:HookScript("OnShow", function()
            if IsSkinningEnabled() then
                -- Check if character pane customization will handle extension
                local SUICore = _G.SuaviUI and _G.SuaviUI.SUICore
                local charSettings = SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.character
                -- Default to true if setting not found (matches SUI_character.lua defaults)
                ...
                -- If charPaneEnabled, SUI_character.lua will call SetCharacterFrameBgExtended(true)
            end
        end)
...
local function RefreshCharacterFrameColors()
    ...
    if ReputationFrame and ReputationFrame.ScrollBox then
        ReputationFrame.ScrollBox:ForEachFrame(function(child)
            ...
            local ReputationBar = child.Content and child.Content.ReputationBar
            if ReputationBar and ReputationBar.suiBackdrop then
                ReputationBar.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, 1)
            end
        end)
    end
...
local function SkinEquipmentManager()
    ...
    local popup = _G.SuaviUI_EquipMgrPopup
...
local function SkinTitleManagerPane()
    ...
    local popup = _G.SuaviUI_TitlesPopup
...
_G.SUI_CharacterFrameSkinning = {
    ...
    -- Skinning functions (called by SUI_character.lua)
    SkinEquipmentManager = SkinEquipmentManager,
    SkinTitleManager = SkinTitleManagerPane,
}

-- Legacy compatibility aliases (deprecated - use SUI_CharacterFrameSkinning table)
_G.SuaviUI_RefreshCharacterFrameColors = RefreshCharacterFrameColors

-- Legacy global function aliases for SUI_character.lua
_G.SUI_SkinEquipmentManager = SkinEquipmentManager
_G.SUI_SkinTitleManager = SkinTitleManagerPane
_G.SUI_SetCharacterFrameBgExtended = SetCharacterFrameBgExtended
