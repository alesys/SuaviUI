...
-- SETUP HELPERS
---------------------------------------------------------------------------
function SUI_Anchoring:SetHelpers(helpers)
    Helpers = helpers or {}
end

-- Helper function wrappers (with fallbacks)
local function Scale(x)
    return Helpers.Scale and Helpers.Scale(x) or (SUICore and SUICore.Scale and SUICore:Scale(x) or x)
end
...
function SUI_Anchoring:RegisterAnchorTarget(name, frame, options)
...
function SUI_Anchoring:UnregisterAnchorTarget(name)
...
function SUI_Anchoring:GetAnchorTarget(name)
...
function SUI_Anchoring:GetAnchorTargetList(include, exclude, excludeSelf)
    include = include or {}
    exclude = exclude or {}
...
function SUI_Anchoring:GetAnchorDimensions(anchorFrame, anchorTargetName)
...
function SUI_Anchoring:PositionFrame(frame, anchorTarget, anchorPoint, offsetX, offsetY, parentFrame, options)
...
function SUI_Anchoring:GetAnchorTargetName(frame)
...
function SUI_Anchoring:CheckCircularDependency(frame, anchorTarget)
...
function SUI_Anchoring:RegisterAnchoredFrame(frame, config)
...
function SUI_Anchoring:UnregisterAnchoredFrame(frame)
    if not frame then return false end
...
function SUI_Anchoring:SnapTo(frame, anchorTarget, anchorPoint, offsetX, offsetY, options)
...
function SUI_Anchoring:UpdateAllAnchoredFrames()
    if InCombatLockdown() then 
...
function SUI_Anchoring:UpdateFramesForTarget(anchorTargetName)
    if InCombatLockdown() then 
        -- Defer update after combat
        C_Timer.After(0, function()
...
_G.SuaviUI_UpdateAnchoredFrames = function()
    if SUI_Anchoring then
        SUI_Anchoring:UpdateAllAnchoredFrames()
    end
end

-- Backward compatibility alias
_G.SuaviUI_UpdateAnchoredUnitFrames = _G.SuaviUI_UpdateAnchoredFrames
_G.SuaviUI_UpdateCDMAnchoredUnitFrames = _G.SuaviUI_UpdateAnchoredFrames
