...
local function CreateGeneralQoLPage(parent)
    ...
    local function RefreshAll()
        ...
        C_Timer.After(0.1, function()
            if SUICore and SUICore.ApplyViewerLayout then
                SUICore:ApplyViewerLayout("EssentialCooldownViewer")
                SUICore:ApplyViewerLayout("UtilityCooldownViewer")
            end
        end)
    end
...
local function CreateCDMSetupPage(parent)
    ...
    local function BuildEssentialTab(tabContent)
        ...
        -- Set search context for auto-registration
        GUI:SetSearchContext({tabIndex = 6, tabName = "CDM Setup", subTabIndex = 1, subTabName = "Essential"})
    ...
    local function BuildUtilityTab(tabContent)
        ...
        -- Set search context for auto-registration
        GUI:SetSearchContext({tabIndex = 6, tabName = "CDM Setup", subTabIndex = 2, subTabName = "Utility"})
    ...
    local function BuildBuffTab(tabContent)
        ...
        GUI:SetSearchContext({tabIndex = 6, tabName = "CDM Setup", subTabIndex = 3, subTabName = "Buff"})
        ...
        if trackedData.texture == nil then trackedData.texture = "Suavi v5" end
...
---------------------------------------------------------------------------
-- PAGE: Class Resource Bars (standalone main tab)
---------------------------------------------------------------------------
local function CreateClassResourceBarsPage(parent)
    local scroll, content = CreateScrollableContent(parent)
    local db = GetDB()
    local y = -10
    local PAD = 10
    ...
    -- GENERAL SETTINGS
    -- =====================================================
    GUI:SetSearchSection("General")
    local generalHeader = GUI:CreateSectionHeader(content, "General")
    generalHeader:SetPoint("TOPLEFT", PAD, y)
    y = y - generalHeader.gap
    ...
    -- Enable toggles
    local enablePrimary = GUI:CreateFormToggle(content, "Enable Primary Class Resource Bar", "enabled", primary, PromptResourceBarReload)
    enablePrimary:SetPoint("TOPLEFT", PAD, y)
    enablePrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local enableSecondary = GUI:CreateFormToggle(content, "Enable Secondary Class Resource Bar", "enabled", secondary, PromptResourceBarReload)
    enableSecondary:SetPoint("TOPLEFT", PAD, y)
    enableSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Standalone toggles
    local standalonePrimary = GUI:CreateFormToggle(content, "Primary Standalone Mode", "standaloneMode", primary, PromptResourceBarReload)
    standalonePrimary:SetPoint("TOPLEFT", PAD, y)
    standalonePrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local standaloneSecondary = GUI:CreateFormToggle(content, "Secondary Standalone Mode", "standaloneMode", secondary, PromptResourceBarReload)
    standaloneSecondary:SetPoint("TOPLEFT", PAD, y)
    standaloneSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local secondaryImptText = GUI:CreateLabel(content, "IMPT: If you choose NOT to display a Primary Bar, and ONLY want a Secondary Bar, toggle this ON. Else it will not show.", 11, C.warning)
    secondaryImptText:SetPoint("TOPLEFT", PAD, y)
    secondaryImptText:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    secondaryImptText:SetJustifyH("LEFT")
    y = y - 25

    local standaloneDesc = GUI:CreateLabel(content, "Standalone Mode: Bar won't fade or hide with CDM visibility. Use if you don't use Essential/Utility cooldown displays.", 11, C.textMuted)
    standaloneDesc:SetPoint("TOPLEFT", PAD, y)
    standaloneDesc:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    standaloneDesc:SetJustifyH("LEFT")
    y = y - 25

    -- Unthrottled CPU Use toggle (affects both primary and secondary)
    local unthrottledToggle = GUI:CreateFormToggle(content, "Unthrottled CPU Use", "unthrottledCPU", primary, RefreshPowerBars)
    unthrottledToggle:SetPoint("TOPLEFT", PAD, y)
    unthrottledToggle:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local unthrottledDesc = GUI:CreateLabel(content, "Remove throttle on the number of updates per second. Toggle on for smoother updates, but higher CPU Usage.", 11, C.textMuted)
    unthrottledDesc:SetPoint("TOPLEFT", PAD, y)
    unthrottledDesc:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    ...
    -- =====================================================
    GUI:SetSearchSection("Primary Class Resource Bar")
    local primaryHeader = GUI:CreateSectionHeader(content, "Primary Class Resource Bar")
    primaryHeader:SetPoint("TOPLEFT", PAD, y)
    y = y - primaryHeader.gap

    local primaryDesc = GUI:CreateLabel(content, "Customize individual resource colors in the Resource Colors section at the bottom. Applied when 'Use Resource Type Color' is enabled.", 11, C.textMuted)
    primaryDesc:SetPoint("TOPLEFT", PAD, y)
    primaryDesc:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    primaryDesc:SetJustifyH("LEFT")
    y = y - 20

    local primaryWarning = GUI:CreateLabel(content, "Designed for horizontal layouts used by most players. Vertical mode requires extra setup (row offsets, orientation toggles).", 11, C.warning)
    primaryWarning:SetPoint("TOPLEFT", PAD, y)
    primaryWarning:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    ...
    local orientationPrimary = GUI:CreateFormDropdown(content, "Orientation", orientationOptions, "orientation", primary, RefreshPowerBars)
    orientationPrimary:SetPoint("TOPLEFT", PAD, y)
    orientationPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Align To dropdown (aligns position to CDM frames)
    local alignToOptions = {
        {value = "none", text = "None"},
        {value = "essential", text = "Essential CDs"},
        {value = "utility", text = "Utility CDs"},
    }
    local alignToPrimary = GUI:CreateFormDropdown(content, "Align To", alignToOptions, "alignTo", primary, function()
        -- When aligning, snap to the selected CDM position
        local target = primary.alignTo
        if target == "none" then
            RefreshPowerBars()
            return
        end
        ...
        local viewer = (target == "essential") and _G.EssentialCooldownViewer or _G.UtilityCooldownViewer
        if viewer and viewer:IsShown() then
            ...
            if rawCenterX and rawCenterY and rawScreenX and rawScreenY then
                ...
                if isVertical then
                    ...
                    local borderSize = (target == "essential") and (viewer.__cdmBottomRowBorderSize or 0) or (viewer.__cdmRow1BorderSize or 0)

                    if target == "essential" then
                        -- Right of Essential
                        local cdmVisualRight = viewerCenterX + (totalWidth / 2) + borderSize
                        ...
                        -- Left of Utility
                        local cdmVisualLeft = viewerCenterX - (totalWidth / 2) - borderSize
                    ...
                    primary.offsetY = math.floor(viewerCenterY - screenCenterY + 0.5)
                else
                    local totalHeight = viewer.__cdmTotalHeight or viewer:GetHeight() or 100
                    ...
                    primary.offsetX = math.floor(viewerCenterX - screenCenterX + 0.5)
                end
                ...
                print("|cFF56D1FFSuaviUI:|r Could not get screen positions. Try again.")
            end
        else
            print("|cFF56D1FFSuaviUI:|r " .. (target == "essential" and "Essential" or "Utility") .. " Cooldowns viewer not found or not visible.")
        end
    end)
    alignToPrimary:SetPoint("TOPLEFT", PAD, y)
    alignToPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Width Sync dropdown (syncs width to CDM frames dynamically)
    local widthSyncOptions = {
        {value = "none", text = "None"},
        {value = "essential", text = "Essential CDs"},
        {value = "utility", text = "Utility CDs"},
    }

    local function UpdateWidthSliderState()
        if widthPrimarySlider and widthPrimarySlider.SetEnabled then
            widthPrimarySlider:SetEnabled(primary.widthSync == "none")
        end
    end

    local widthSyncPrimary = GUI:CreateFormDropdown(content, "Width Sync", widthSyncOptions, "widthSync", primary, function()
        local target = primary.widthSync
        if target == "none" then
        ...
        if viewer and viewer:IsShown() then
            ...
            if isVertical then
                local totalHeight = viewer.__cdmTotalHeight or viewer:GetHeight() or 100
                local topBottomBorderSize = viewer.__cdmRow1BorderSize or 0
                newWidth = totalHeight + (2 * topBottomBorderSize) - (2 * barBorderSize)
            else
                local rowWidth
                if target == "essential" then
                    rowWidth = viewer.__cdmRow1Width or viewer.__cdmIconWidth or 300
                else
                    rowWidth = viewer.__cdmBottomRowWidth or viewer.__cdmIconWidth or 300
                end
                local borderSize = (target == "essential") and (viewer.__cdmRow1BorderSize or 2) or (viewer.__cdmBottomRowBorderSize or 2)
                newWidth = rowWidth + (2 * borderSize) - (2 * barBorderSize)
            end

            primary.width = math.floor(newWidth + 0.5)
            primary.autoAttach = false
            primary.useRawPixels = true
    ...
    widthSyncPrimary:SetPoint("TOPLEFT", PAD, y)
    widthSyncPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    UpdateWidthSliderState()
    y = y - FORM_ROW
    ...
    local powerColorPrimary = GUI:CreateFormCheckbox(content, "Use Resource Type Color", "usePowerColor", primary, function()
    ...
    powerColorPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local resourceColorDescPrimary = GUI:CreateLabel(content, "Uses per-resource colors from the Resource Colors section below.", 11)
    resourceColorDescPrimary:SetPoint("TOPLEFT", PAD, y + 4)
    resourceColorDescPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    resourceColorDescPrimary:SetJustifyH("LEFT")
    resourceColorDescPrimary:SetTextColor(0.6, 0.6, 0.6)
    y = y - FORM_ROW

    local classColorPrimary = GUI:CreateFormCheckbox(content, "Use Class Color", "useClassColor", primary, function()
    ...
    classColorPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local bgColorPrimary = GUI:CreateFormColorPicker(content, "Background Color", "bgColor", primary, RefreshPowerBars)
    bgColorPrimary:SetPoint("TOPLEFT", PAD, y)
    bgColorPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local customColorOverridePrimary = GUI:CreateFormCheckbox(content, "Custom Color Override", "useCustomColor", primary, function()
    ...
    customColorOverridePrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    customColorPickerPrimary = GUI:CreateFormColorPicker(content, "Custom Color", "customColor", primary, RefreshPowerBars)
    customColorPickerPrimary:SetPoint("TOPLEFT", PAD, y)
    customColorPickerPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    customColorPickerPrimary:SetEnabled(primary.useCustomColor)
    y = y - FORM_ROW

    -- Text display options
    local showTextPrimary = GUI:CreateFormCheckbox(content, "Show Number", "showText", primary, RefreshPowerBars)
    showTextPrimary:SetPoint("TOPLEFT", PAD, y)
    showTextPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local showPercentPrimary = GUI:CreateFormCheckbox(content, "Show as Percent", "showPercent", primary, RefreshPowerBars)
    showPercentPrimary:SetPoint("TOPLEFT", PAD, y)
    showPercentPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Tick marks
    local showTicksPrimary = GUI:CreateFormCheckbox(content, "Show Tick Marks", "showTicks", primary, RefreshPowerBars)
    showTicksPrimary:SetPoint("TOPLEFT", PAD, y)
    showTicksPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local tickThicknessPrimary = GUI:CreateFormSlider(content, "Tick Thickness", 1, 4, 1, "tickThickness", primary, RefreshPowerBars)
    tickThicknessPrimary:SetPoint("TOPLEFT", PAD, y)
    tickThicknessPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local tickColorPrimary = GUI:CreateFormColorPicker(content, "Tick Color", "tickColor", primary, RefreshPowerBars)
    tickColorPrimary:SetPoint("TOPLEFT", PAD, y)
    tickColorPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Size sliders (form style)
    widthPrimarySlider = GUI:CreateFormSlider(content, "Width", 0, 2000, 1, "width", primary, RefreshPowerBars)
    widthPrimarySlider:SetPoint("TOPLEFT", PAD, y)
    widthPrimarySlider:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    widthPrimarySlider:SetEnabled(primary.widthSync == "none")  -- Disabled when synced
    y = y - FORM_ROW

    local heightPrimary = GUI:CreateFormSlider(content, "Height", 1, 100, 1, "height", primary, RefreshPowerBars)
    heightPrimary:SetPoint("TOPLEFT", PAD, y)
    heightPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local borderPrimary = GUI:CreateFormSlider(content, "Border Size", 0, 8, 1, "borderSize", primary, RefreshPowerBars)
    borderPrimary:SetPoint("TOPLEFT", PAD, y)
    borderPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Position sliders
    local xOffsetPrimarySlider = GUI:CreateFormSlider(content, "X Offset", -1000, 1000, 1, "offsetX", primary, RefreshPowerBars)
    xOffsetPrimarySlider:SetPoint("TOPLEFT", PAD, y)
    xOffsetPrimarySlider:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    yOffsetPrimarySlider = GUI:CreateFormSlider(content, "Y Offset", -1000, 1000, 1, "offsetY", primary, RefreshPowerBars)
    yOffsetPrimarySlider:SetPoint("TOPLEFT", PAD, y)
    yOffsetPrimarySlider:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Register sliders for real-time sync during Edit Mode
    if _G.SuaviUI and _G.SuaviUI.SUICore and _G.SuaviUI.SUICore.RegisterPowerBarEditModeSliders then
        _G.SuaviUI.SUICore:RegisterPowerBarEditModeSliders("primary", xOffsetPrimarySlider, yOffsetPrimarySlider)
    end

    -- Text sliders
    local textSizePrimary = GUI:CreateFormSlider(content, "Text Size", 8, 50, 1, "textSize", primary, RefreshPowerBars)
    textSizePrimary:SetPoint("TOPLEFT", PAD, y)
    textSizePrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local textXPrimary = GUI:CreateFormSlider(content, "Text X Offset", -500, 500, 1, "textX", primary, RefreshPowerBars)
    textXPrimary:SetPoint("TOPLEFT", PAD, y)
    textXPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local textYPrimary = GUI:CreateFormSlider(content, "Text Y Offset", -500, 500, 1, "textY", primary, RefreshPowerBars)
    textYPrimary:SetPoint("TOPLEFT", PAD, y)
    textYPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Text color settings
    local textCustomColorPrimary  -- Forward declare for mutual reference

    local textUseClassColorPrimary = GUI:CreateFormCheckbox(content, "Use Class Color for Text", "textUseClassColor", primary, function()
    ...
    textUseClassColorPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    textCustomColorPrimary = GUI:CreateFormColorPicker(content, "Custom Text Color", "textCustomColor", primary, RefreshPowerBars)
    textCustomColorPrimary:SetPoint("TOPLEFT", PAD, y)
    textCustomColorPrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    textCustomColorPrimary:SetEnabled(not primary.textUseClassColor)  -- Initial state
    y = y - FORM_ROW

    local texturePrimary = GUI:CreateFormDropdown(content, "Bar Texture", GetTextureList(), "texture", primary, RefreshPowerBars)
    texturePrimary:SetPoint("TOPLEFT", PAD, y)
    texturePrimary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW
    ...
    -- =====================================================
    GUI:SetSearchSection("Secondary Class Resource Bar")
    local secondaryHeader = GUI:CreateSectionHeader(content, "Secondary Class Resource Bar")
    secondaryHeader:SetPoint("TOPLEFT", PAD, y)
    y = y - secondaryHeader.gap

    local secondaryDesc = GUI:CreateLabel(content, "Customize individual resource colors in the Resource Colors section at the bottom. Applied when 'Use Resource Type Color' is enabled.", 11, C.textMuted)
    secondaryDesc:SetPoint("TOPLEFT", PAD, y)
    secondaryDesc:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    secondaryDesc:SetJustifyH("LEFT")
    y = y - 20

    local secondaryWarning = GUI:CreateLabel(content, "Designed for horizontal layouts used by most players. Vertical mode requires extra setup (row offsets, orientation toggles).", 11, C.warning)
    secondaryWarning:SetPoint("TOPLEFT", PAD, y)
    secondaryWarning:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    ...
    local orientationSecondary = GUI:CreateFormDropdown(content, "Orientation", orientationOptionsSecondary, "orientation", secondary, RefreshPowerBars)
    orientationSecondary:SetPoint("TOPLEFT", PAD, y)
    orientationSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Align To dropdown (aligns position to CDM frames or Primary)
    local alignToOptionsSecondary = {
        {value = "none", text = "None"},
        {value = "essential", text = "Essential CDs"},
        {value = "utility", text = "Utility CDs"},
        {value = "primary", text = "Primary Bar"},
    }
    local alignToSecondary = GUI:CreateFormDropdown(content, "Align To", alignToOptionsSecondary, "alignTo", secondary, function()
        local target = secondary.alignTo
        if target == "none" then
            RefreshPowerBars()
            return
        end
        ...
        if target == "primary" then
            -- Align to Primary bar
            local SUICore = _G.SuaviUI and _G.SuaviUI.SUICore
            local primaryBar = SUICore and SUICore.powerBar
            local primaryCfg = SUICore and SUICore.db and SUICore.db.profile.powerBar
            if primaryBar and primaryBar:IsShown() and primaryCfg then
                ...
                print("|cFF56D1FFSuaviUI:|r Primary Class Resource Bar not found or not visible.")
            end
            return
        end

        -- Align to CDM (Essential or Utility)
        if _G.SuaviUI_RefreshNCDM then
            _G.SuaviUI_RefreshNCDM()
        end
        ...
        if viewer and viewer:IsShown() then
            ...
            if rawCenterX and rawCenterY and rawScreenX and rawScreenY then
                ...
                if isVertical then
                    local totalHeight = viewer.__cdmTotalHeight or viewer:GetHeight() or 100
                    local totalWidth = viewer.__cdmIconWidth or viewer:GetWidth()
                    local borderSize = viewer.__cdmRow1BorderSize or 0
                    ...
                    secondary.lockedBaseY = math.floor(viewerCenterY - screenCenterY + 0.5)
                else
                    local totalHeight = viewer.__cdmTotalHeight or viewer:GetHeight() or 100
                    ...
                    secondary.lockedBaseX = math.floor(viewerCenterX - screenCenterX + 0.5)
                end
                ...
                if yOffsetSecondarySlider and yOffsetSecondarySlider.SetValue then
                    yOffsetSecondarySlider.SetValue(secondary.offsetY, true)
                end
            else
                print("|cFF56D1FFSuaviUI:|r Could not get screen positions. Try again.")
            end
    ...
    alignToSecondary:SetPoint("TOPLEFT", PAD, y)
    alignToSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Width Sync dropdown (syncs width to CDM frames or Primary dynamically)
    local widthSyncOptionsSecondary = {
        {value = "none", text = "None"},
        {value = "essential", text = "Essential CDs"},
        {value = "utility", text = "Utility CDs"},
        {value = "primary", text = "Primary Bar"},
    ...
    local function UpdateSecWidthSliderState()
        if widthSecondarySlider and widthSecondarySlider.SetEnabled then
            widthSecondarySlider:SetEnabled(secondary.widthSync == "none")
        end
    end

    local widthSyncSecondary = GUI:CreateFormDropdown(content, "Width Sync", widthSyncOptionsSecondary, "widthSync", secondary, function()
        local target = secondary.widthSync
        if target == "none" then
            UpdateSecWidthSliderState()
            RefreshPowerBars()
            return
        end

        if target == "primary" then
            -- Sync to Primary bar width
            local SUICore = _G.SuaviUI and _G.SuaviUI.SUICore
            local primaryBar = SUICore and SUICore.powerBar
            local primaryCfg = SUICore and SUICore.db and SUICore.db.profile.powerBar
            if primaryBar and primaryBar:IsShown() and primaryCfg then
                ...
                if widthSecondarySlider and widthSecondarySlider.SetValue then
                    widthSecondarySlider.SetValue(secondary.width, true)
                end
            else
                print("|cFF56D1FFSuaviUI:|r Primary Class Resource Bar not found or not visible.")
            end
        ...
        if viewer and viewer:IsShown() then
            ...
            secondary.width = math.floor(newWidth + 0.5)
            secondary.autoAttach = false
            secondary.useRawPixels = true
    ...
    widthSyncSecondary:SetPoint("TOPLEFT", PAD, y)
    widthSyncSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    UpdateSecWidthSliderState()
    y = y - FORM_ROW
    ...
    local powerColorSecondary = GUI:CreateFormCheckbox(content, "Use Resource Type Color", "usePowerColor", secondary, function()
    ...
    powerColorSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local resourceColorDescSecondary = GUI:CreateLabel(content, "Uses per-resource colors from the Resource Colors section below.", 11)
    resourceColorDescSecondary:SetPoint("TOPLEFT", PAD, y + 4)
    resourceColorDescSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    resourceColorDescSecondary:SetJustifyH("LEFT")
    resourceColorDescSecondary:SetTextColor(0.6, 0.6, 0.6)
    y = y - FORM_ROW

    local classColorSecondary = GUI:CreateFormCheckbox(content, "Use Class Color", "useClassColor", secondary, function()
    ...
    classColorSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local bgColorSecondary = GUI:CreateFormColorPicker(content, "Background Color", "bgColor", secondary, RefreshPowerBars)
    bgColorSecondary:SetPoint("TOPLEFT", PAD, y)
    bgColorSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local customColorOverrideSecondary = GUI:CreateFormCheckbox(content, "Custom Color Override", "useCustomColor", secondary, function()
    ...
    customColorOverrideSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    customColorPickerSecondary = GUI:CreateFormColorPicker(content, "Custom Color", "customColor", secondary, RefreshPowerBars)
    customColorPickerSecondary:SetPoint("TOPLEFT", PAD, y)
    customColorPickerSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    customColorPickerSecondary:SetEnabled(secondary.useCustomColor)
    y = y - FORM_ROW

    -- Text display options
    local showTextSecondary = GUI:CreateFormCheckbox(content, "Show Number", "showText", secondary, RefreshPowerBars)
    showTextSecondary:SetPoint("TOPLEFT", PAD, y)
    showTextSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local showPercentSecondary = GUI:CreateFormCheckbox(content, "Show as Percent", "showPercent", secondary, RefreshPowerBars)
    showPercentSecondary:SetPoint("TOPLEFT", PAD, y)
    showPercentSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local showRuneTextSecondary = GUI:CreateFormCheckbox(content, "Show Rune CD Text (DKs)", "showFragmentedPowerBarText", secondary, RefreshPowerBars)
    showRuneTextSecondary:SetPoint("TOPLEFT", PAD, y)
    showRuneTextSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    local _, playerClass = UnitClass("player")
    showRuneTextSecondary:SetEnabled(playerClass == "DEATHKNIGHT")
    y = y - FORM_ROW

    -- Tick marks
    local showTicksSecondary = GUI:CreateFormCheckbox(content, "Show Tick Marks", "showTicks", secondary, RefreshPowerBars)
    showTicksSecondary:SetPoint("TOPLEFT", PAD, y)
    showTicksSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local tickThicknessSecondary = GUI:CreateFormSlider(content, "Tick Thickness", 1, 4, 1, "tickThickness", secondary, RefreshPowerBars)
    tickThicknessSecondary:SetPoint("TOPLEFT", PAD, y)
    tickThicknessSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local tickColorSecondary = GUI:CreateFormColorPicker(content, "Tick Color", "tickColor", secondary, RefreshPowerBars)
    tickColorSecondary:SetPoint("TOPLEFT", PAD, y)
    tickColorSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Size sliders (form style)
    widthSecondarySlider = GUI:CreateFormSlider(content, "Width", 0, 2000, 1, "width", secondary, RefreshPowerBars)
    widthSecondarySlider:SetPoint("TOPLEFT", PAD, y)
    widthSecondarySlider:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local heightSecondary = GUI:CreateFormSlider(content, "Height", 1, 100, 1, "height", secondary, RefreshPowerBars)
    heightSecondary:SetPoint("TOPLEFT", PAD, y)
    heightSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local borderSecondary = GUI:CreateFormSlider(content, "Border Size", 0, 8, 1, "borderSize", secondary, RefreshPowerBars)
    borderSecondary:SetPoint("TOPLEFT", PAD, y)
    borderSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Position sliders
    local xOffsetSecondarySlider = GUI:CreateFormSlider(content, "X Offset", -1000, 1000, 1, "offsetX", secondary, RefreshPowerBars)
    xOffsetSecondarySlider:SetPoint("TOPLEFT", PAD, y)
    xOffsetSecondarySlider:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    yOffsetSecondarySlider = GUI:CreateFormSlider(content, "Y Offset", -1000, 1000, 1, "offsetY", secondary, RefreshPowerBars)
    yOffsetSecondarySlider:SetPoint("TOPLEFT", PAD, y)
    yOffsetSecondarySlider:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Register sliders for real-time sync during Edit Mode
    if _G.SuaviUI and _G.SuaviUI.SUICore and _G.SuaviUI.SUICore.RegisterPowerBarEditModeSliders then
        _G.SuaviUI.SUICore:RegisterPowerBarEditModeSliders("secondary", xOffsetSecondarySlider, yOffsetSecondarySlider)
    end

    -- Text sliders
    local textSizeSecondary = GUI:CreateFormSlider(content, "Text Size", 8, 50, 1, "textSize", secondary, RefreshPowerBars)
    textSizeSecondary:SetPoint("TOPLEFT", PAD, y)
    textSizeSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local textXSecondary = GUI:CreateFormSlider(content, "Text X Offset", -500, 500, 1, "textX", secondary, RefreshPowerBars)
    textXSecondary:SetPoint("TOPLEFT", PAD, y)
    textXSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local textYSecondary = GUI:CreateFormSlider(content, "Text Y Offset", -500, 500, 1, "textY", secondary, RefreshPowerBars)
    textYSecondary:SetPoint("TOPLEFT", PAD, y)
    textYSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Text color settings
    local textCustomColorSecondary  -- Forward declare for mutual reference

    local textUseClassColorSecondary = GUI:CreateFormCheckbox(content, "Use Class Color for Text", "textUseClassColor", secondary, function()
    ...
    textUseClassColorSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    textCustomColorSecondary = GUI:CreateFormColorPicker(content, "Custom Text Color", "textCustomColor", secondary, RefreshPowerBars)
    textCustomColorSecondary:SetPoint("TOPLEFT", PAD, y)
    textCustomColorSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    textCustomColorSecondary:SetEnabled(not secondary.textUseClassColor)  -- Initial state
    y = y - FORM_ROW

    local textureSecondary = GUI:CreateFormDropdown(content, "Bar Texture", GetTextureList(), "texture", secondary, RefreshPowerBars)
    textureSecondary:SetPoint("TOPLEFT", PAD, y)
    textureSecondary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW
    ...
    GUI:SetSearchSection("Tertiary Power Bar")
    local tertiaryHeader = GUI:CreateSectionHeader(content, "Tertiary Power Bar (Evoker Ebon Might)")
    tertiaryHeader:SetPoint("TOPLEFT", PAD, y)
    y = y - tertiaryHeader.gap

    local tertiaryDesc = GUI:CreateLabel(content, "Displays Evoker Ebon Might buff duration. Only available for Evoker Augmentation spec.", 11, C.textMuted)
    tertiaryDesc:SetPoint("TOPLEFT", PAD, y)
    tertiaryDesc:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    ...
    local enableTertiary = GUI:CreateFormToggle(content, "Enable Tertiary Power Bar", "enabled", tertiary, PromptResourceBarReload)
    enableTertiary:SetPoint("TOPLEFT", PAD, y)
    enableTertiary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Tertiary size sliders
    local widthTertiarySlider = GUI:CreateFormSlider(content, "Width", 0, 2000, 1, "width", tertiary, RefreshPowerBars)
    widthTertiarySlider:SetPoint("TOPLEFT", PAD, y)
    widthTertiarySlider:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local heightTertiary = GUI:CreateFormSlider(content, "Height", 1, 100, 1, "height", tertiary, RefreshPowerBars)
    heightTertiary:SetPoint("TOPLEFT", PAD, y)
    heightTertiary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local borderTertiary = GUI:CreateFormSlider(content, "Border Size", 0, 8, 1, "borderSize", tertiary, RefreshPowerBars)
    borderTertiary:SetPoint("TOPLEFT", PAD, y)
    borderTertiary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Tertiary position sliders
    local xOffsetTertiarySlider = GUI:CreateFormSlider(content, "X Offset", -1000, 1000, 1, "offsetX", tertiary, RefreshPowerBars)
    xOffsetTertiarySlider:SetPoint("TOPLEFT", PAD, y)
    xOffsetTertiarySlider:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local yOffsetTertiarySlider = GUI:CreateFormSlider(content, "Y Offset", -1000, 1000, 1, "offsetY", tertiary, RefreshPowerBars)
    yOffsetTertiarySlider:SetPoint("TOPLEFT", PAD, y)
    yOffsetTertiarySlider:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Tertiary text options
    local showTextTertiary = GUI:CreateFormCheckbox(content, "Show Duration Text", "showText", tertiary, RefreshPowerBars)
    showTextTertiary:SetPoint("TOPLEFT", PAD, y)
    showTextTertiary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local textSizeTertiary = GUI:CreateFormSlider(content, "Text Size", 8, 50, 1, "textSize", tertiary, RefreshPowerBars)
    textSizeTertiary:SetPoint("TOPLEFT", PAD, y)
    textSizeTertiary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    -- Tertiary colors
    local bgColorTertiary = GUI:CreateFormColorPicker(content, "Background Color", "bgColor", tertiary, RefreshPowerBars)
    bgColorTertiary:SetPoint("TOPLEFT", PAD, y)
    bgColorTertiary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local customColorTertiary = GUI:CreateFormColorPicker(content, "Bar Color", "customColor", tertiary, RefreshPowerBars)
    customColorTertiary:SetPoint("TOPLEFT", PAD, y)
    customColorTertiary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local textCustomColorTertiary = GUI:CreateFormColorPicker(content, "Text Color", "textCustomColor", tertiary, RefreshPowerBars)
    textCustomColorTertiary:SetPoint("TOPLEFT", PAD, y)
    textCustomColorTertiary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local textureTertiary = GUI:CreateFormDropdown(content, "Bar Texture", GetTextureList(), "texture", tertiary, RefreshPowerBars)
    textureTertiary:SetPoint("TOPLEFT", PAD, y)
    textureTertiary:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW
    ...
    GUI:SetSearchSection("Resource Colors")
    local powerColorsHeader = GUI:CreateSectionHeader(content, "Reset Resource Bar Colors To Default")
    ...
    local resetPowerColorsContainer = CreateFrame("Frame", nil, content)
    resetPowerColorsContainer:SetHeight(FORM_ROW)
    resetPowerColorsContainer:SetPoint("TOPLEFT", PAD, y)
    resetPowerColorsContainer:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    ...
    resetPowerColorsBtn:SetScript("OnClick", function()
        ...
        RefreshPowerBars()
        print("|cFF56D1FFSuaviUI:|r Resource colors reset to defaults.")
    end)
    y = y - FORM_ROW
    ...
    -- =====================================================
    y = y - 8
    local coreHeader = GUI:CreateSectionHeader(content, "Bar Colors for Core Resources")
    coreHeader:SetPoint("TOPLEFT", PAD, y)
    y = y - coreHeader.gap

    local rageColor = GUI:CreateFormColorPicker(content, "Rage", "rage", pc, RefreshPowerBars)
    rageColor:SetPoint("TOPLEFT", PAD, y)
    rageColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    rageColor.dbKey = "rage"
    table.insert(powerColorWidgets, rageColor)
    y = y - FORM_ROW

    local energyColor = GUI:CreateFormColorPicker(content, "Energy", "energy", pc, RefreshPowerBars)
    energyColor:SetPoint("TOPLEFT", PAD, y)
    energyColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    energyColor.dbKey = "energy"
    table.insert(powerColorWidgets, energyColor)
    y = y - FORM_ROW

    local manaColor = GUI:CreateFormColorPicker(content, "Mana", "mana", pc, RefreshPowerBars)
    manaColor:SetPoint("TOPLEFT", PAD, y)
    manaColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    manaColor.dbKey = "mana"
    table.insert(powerColorWidgets, manaColor)
    y = y - FORM_ROW

    local focusColor = GUI:CreateFormColorPicker(content, "Focus", "focus", pc, RefreshPowerBars)
    focusColor:SetPoint("TOPLEFT", PAD, y)
    focusColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    focusColor.dbKey = "focus"
    table.insert(powerColorWidgets, focusColor)
    y = y - FORM_ROW

    local runicPowerColor = GUI:CreateFormColorPicker(content, "Runic Power", "runicPower", pc, RefreshPowerBars)
    runicPowerColor:SetPoint("TOPLEFT", PAD, y)
    runicPowerColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    runicPowerColor.dbKey = "runicPower"
    table.insert(powerColorWidgets, runicPowerColor)
    y = y - FORM_ROW

    local furyColor = GUI:CreateFormColorPicker(content, "Fury", "fury", pc, RefreshPowerBars)
    furyColor:SetPoint("TOPLEFT", PAD, y)
    furyColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    furyColor.dbKey = "fury"
    table.insert(powerColorWidgets, furyColor)
    y = y - FORM_ROW

    local insanityColor = GUI:CreateFormColorPicker(content, "Insanity", "insanity", pc, RefreshPowerBars)
    insanityColor:SetPoint("TOPLEFT", PAD, y)
    insanityColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    insanityColor.dbKey = "insanity"
    table.insert(powerColorWidgets, insanityColor)
    y = y - FORM_ROW

    local maelstromColor = GUI:CreateFormColorPicker(content, "Maelstrom", "maelstrom", pc, RefreshPowerBars)
    maelstromColor:SetPoint("TOPLEFT", PAD, y)
    maelstromColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    maelstromColor.dbKey = "maelstrom"
    table.insert(powerColorWidgets, maelstromColor)
    y = y - FORM_ROW

    local lunarPowerColor = GUI:CreateFormColorPicker(content, "Astral Power", "lunarPower", pc, RefreshPowerBars)
    lunarPowerColor:SetPoint("TOPLEFT", PAD, y)
    lunarPowerColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    ...
    -- =====================================================
    y = y - 8
    local builderHeader = GUI:CreateSectionHeader(content, "Bar Colors for Builder Resources")
    builderHeader:SetPoint("TOPLEFT", PAD, y)
    y = y - builderHeader.gap

    local holyPowerColor = GUI:CreateFormColorPicker(content, "Holy Power", "holyPower", pc, RefreshPowerBars)
    holyPowerColor:SetPoint("TOPLEFT", PAD, y)
    holyPowerColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    holyPowerColor.dbKey = "holyPower"
    table.insert(powerColorWidgets, holyPowerColor)
    y = y - FORM_ROW

    local chiColor = GUI:CreateFormColorPicker(content, "Chi", "chi", pc, RefreshPowerBars)
    chiColor:SetPoint("TOPLEFT", PAD, y)
    chiColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    chiColor.dbKey = "chi"
    table.insert(powerColorWidgets, chiColor)
    y = y - FORM_ROW

    local comboPointsColor = GUI:CreateFormColorPicker(content, "Combo Points", "comboPoints", pc, RefreshPowerBars)
    comboPointsColor:SetPoint("TOPLEFT", PAD, y)
    comboPointsColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    comboPointsColor.dbKey = "comboPoints"
    table.insert(powerColorWidgets, comboPointsColor)
    y = y - FORM_ROW

    local soulShardsColor = GUI:CreateFormColorPicker(content, "Soul Shards", "soulShards", pc, RefreshPowerBars)
    soulShardsColor:SetPoint("TOPLEFT", PAD, y)
    soulShardsColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    soulShardsColor.dbKey = "soulShards"
    table.insert(powerColorWidgets, soulShardsColor)
    y = y - FORM_ROW

    local arcaneChargesColor = GUI:CreateFormColorPicker(content, "Arcane Charges", "arcaneCharges", pc, RefreshPowerBars)
    arcaneChargesColor:SetPoint("TOPLEFT", PAD, y)
    arcaneChargesColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    arcaneChargesColor.dbKey = "arcaneCharges"
    table.insert(powerColorWidgets, arcaneChargesColor)
    y = y - FORM_ROW

    local essenceColor = GUI:CreateFormColorPicker(content, "Essence", "essence", pc, RefreshPowerBars)
    essenceColor:SetPoint("TOPLEFT", PAD, y)
    essenceColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    ...
    -- =====================================================
    y = y - 8
    local specialHeader = GUI:CreateSectionHeader(content, "Bar Colors for Specialized Resources")
    specialHeader:SetPoint("TOPLEFT", PAD, y)
    y = y - specialHeader.gap

    local staggerColor = GUI:CreateFormColorPicker(content, "Stagger (Fallback)", "stagger", pc, RefreshPowerBars)
    staggerColor:SetPoint("TOPLEFT", PAD, y)
    staggerColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    staggerColor.dbKey = "stagger"
    table.insert(powerColorWidgets, staggerColor)
    y = y - FORM_ROW

    local useStaggerLevels = GUI:CreateFormCheckbox(content, "Use Stagger Level Colors", "useStaggerLevelColors", pc, RefreshPowerBars)
    useStaggerLevels:SetPoint("TOPLEFT", PAD, y)
    useStaggerLevels:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    y = y - FORM_ROW

    local staggerLightColor = GUI:CreateFormColorPicker(content, "Stagger - Light (0-30%)", "staggerLight", pc, RefreshPowerBars)
    staggerLightColor:SetPoint("TOPLEFT", PAD, y)
    staggerLightColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    staggerLightColor.dbKey = "staggerLight"
    table.insert(powerColorWidgets, staggerLightColor)
    y = y - FORM_ROW

    local staggerModerateColor = GUI:CreateFormColorPicker(content, "Stagger - Moderate (30-60%)", "staggerModerate", pc, RefreshPowerBars)
    staggerModerateColor:SetPoint("TOPLEFT", PAD, y)
    staggerModerateColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    staggerModerateColor.dbKey = "staggerModerate"
    table.insert(powerColorWidgets, staggerModerateColor)
    y = y - FORM_ROW

    local staggerHeavyColor = GUI:CreateFormColorPicker(content, "Stagger - Heavy (60%+)", "staggerHeavy", pc, RefreshPowerBars)
    staggerHeavyColor:SetPoint("TOPLEFT", PAD, y)
    staggerHeavyColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    staggerHeavyColor.dbKey = "staggerHeavy"
    table.insert(powerColorWidgets, staggerHeavyColor)
    y = y - FORM_ROW

    local soulFragmentsColor = GUI:CreateFormColorPicker(content, "Soul Fragments", "soulFragments", pc, RefreshPowerBars)
    soulFragmentsColor:SetPoint("TOPLEFT", PAD, y)
    soulFragmentsColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    soulFragmentsColor.dbKey = "soulFragments"
    table.insert(powerColorWidgets, soulFragmentsColor)
    y = y - FORM_ROW

    local runesColor = GUI:CreateFormColorPicker(content, "Runes (Generic)", "runes", pc, RefreshPowerBars)
    runesColor:SetPoint("TOPLEFT", PAD, y)
    runesColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    runesColor.dbKey = "runes"
    table.insert(powerColorWidgets, runesColor)
    y = y - FORM_ROW

    local bloodRunesColor = GUI:CreateFormColorPicker(content, "Blood Runes", "bloodRunes", pc, RefreshPowerBars)
    bloodRunesColor:SetPoint("TOPLEFT", PAD, y)
    bloodRunesColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    bloodRunesColor.dbKey = "bloodRunes"
    table.insert(powerColorWidgets, bloodRunesColor)
    y = y - FORM_ROW

    local frostRunesColor = GUI:CreateFormColorPicker(content, "Frost Runes", "frostRunes", pc, RefreshPowerBars)
    frostRunesColor:SetPoint("TOPLEFT", PAD, y)
    frostRunesColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
    frostRunesColor.dbKey = "frostRunes"
    table.insert(powerColorWidgets, frostRunesColor)
    y = y - FORM_ROW

    local unholyRunesColor = GUI:CreateFormColorPicker(content, "Unholy Runes", "unholyRunes", pc, RefreshPowerBars)
    unholyRunesColor:SetPoint("TOPLEFT", PAD, y)
    unholyRunesColor:SetPoint("RIGHT", content, "RIGHT", -PAD, 0)
...
function GUI:InitializeOptions()
    ...
    GUI:AddTab(frame, "CDM Setup", CreateCDMSetupPage)
    GUI:AddTab(frame, "Class Resource Bars", CreateClassResourceBarsPage)
