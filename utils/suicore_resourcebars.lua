...
local function GetDefaultTexture()
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.general then
        return SUICore.db.profile.general.texture or "Suavi"
    end
    return "Suavi"
end
...
function SUICore:RegisterPowerBarEditModeSliders(barKey, xSlider, ySlider)
...
function SUICore:NotifyPowerBarPositionChanged(barKey, offsetX, offsetY)
...
local function GetResourceColor(resource)
    ...
    local pc = SUICore and SUICore.db and SUICore.db.profile.powerColors
...
local function CreatePowerBarNudgeButton(parent, direction, deltaX, deltaY, barKey)
    ...
    btn:SetScript("OnClick", function()
        local cfg = (barKey == "primary") and SUICore.db.profile.powerBar or SUICore.db.profile.secondaryPowerBar
        ...
        SUICore:NotifyPowerBarPositionChanged(barKey, cfg.offsetX, cfg.offsetY)
    end)
...
function SUICore:EnablePowerBarEditMode()
            ...
            bar:SetScript("OnMouseDown", function(self, button)
                if button == "LeftButton" and PowerBarEditMode.active then
                    if SUICore and SUICore.SelectEditModeElement then
                        SUICore:SelectEditModeElement("powerbar", self._editModeBarKey)
                    end
                end
            end)
                ...
                if PowerBarEditMode.active then
                    ...
                    self:SetScript("OnUpdate", function(frame)
                        ...
                        if selfX and selfY and parentX and parentY then
                            ...
                            -- Notify options panel
                            SUICore:NotifyPowerBarPositionChanged(barKey, offsetX, offsetY)
                        end
                    end)
                end
            end)
...
function SUICore:DisablePowerBarEditMode()
...
function SUICore:UpdatePowerBar()
    local cfg = self.db.profile.powerBar
...
function SUICore:UpdatePowerBarTicks(bar, resource, max)
    local cfg = self.db.profile.powerBar
...
_G.SuaviUI_UpdateLockedPowerBar = function()
    local SUICore = _G.SuaviUI and _G.SuaviUI.SUICore
...
_G.SuaviUI_UpdateLockedPowerBarToUtility = function()
    local SUICore = _G.SuaviUI and _G.SuaviUI.SUICore
    if not SUICore or not SUICore.db then return end
...
_G.SuaviUI_UpdateLockedSecondaryPowerBar = function()
    local SUICore = _G.SuaviUI and _G.SuaviUI.SUICore
    if not SUICore or not SUICore.db then return end
...
_G.SuaviUI_UpdateLockedSecondaryPowerBarToUtility = function()
    local SUICore = _G.SuaviUI and _G.SuaviUI.SUICore
    if not SUICore or not SUICore.db then return end
...
function SUICore:GetSecondaryPowerBar()
...
function SUICore:CreateFragmentedPowerBars(bar, resource, isVertical)
    local cfg = self.db.profile.secondaryPowerBar
...
function SUICore:UpdateSecondaryPowerBarTicks(bar, resource, max)
    local cfg = self.db.profile.secondaryPowerBar
...
function SUICore:UpdateSecondaryPowerBar()
    local cfg = self.db.profile.secondaryPowerBar
    if not cfg.enabled then
...
function SUICore:OnRunePowerUpdate()
    local now = GetTime()
...
local function InitializeResourceBars(self)
    ...
    C_Timer.After(0.6, function()
        if EditModeManagerFrame and not SUICore._powerBarEditModeHooked then
            SUICore._powerBarEditModeHooked = true
            hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function()
                if not InCombatLockdown() then
                    SUICore:EnablePowerBarEditMode()
                end
            end)
            hooksecurefunc(EditModeManagerFrame, "ExitEditMode", function()
                if not InCombatLockdown() then
                    SUICore:DisablePowerBarEditMode()
                end
            end)
        end
    end)
...
function SUICore:OnSpecChanged()
...
function SUICore:OnShapeshiftChanged()
