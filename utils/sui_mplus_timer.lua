...
local function GetSettings()
    ...
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.mplusTimer then
        return SUICore.db.profile.mplusTimer
    end
    return {
        enabled = true,
...
local function GetPosition()
    ...
    local SUICore = _G.SuaviUI and _G.SuaviUI.SUICore
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.mplusTimer then
        local pos = SUICore.db.profile.mplusTimer.position
        if pos then
            return {
...
local function SavePosition(point, relPoint, x, y)
    ...
    if SUICore and SUICore.db and SUICore.db.profile then
        if not SUICore.db.profile.mplusTimer then
            SUICore.db.profile.mplusTimer = {}
        end
        SUICore.db.profile.mplusTimer.position = {
            point = point,
            relPoint = relPoint,
            x = x,
            y = y
...
function MPlusTimer:CreateFrames()
    ...
    -- Root frame
    local root = CreateFrame("Frame", "SUI_MPlusTimerFrame", UIParent)
...
function MPlusTimer:EnableDemoMode()
    if self.state.inChallenge then
        print("|cFF34D4E8[SUI M+ Timer]|r Can't enable demo mode during active M+!")
        return
    end
    ...
    print("|cFF34D4E8[SUI M+ Timer]|r Demo mode enabled. Type /qmpt demo to disable.")
end
...
function MPlusTimer:DisableDemoMode()
    ...
    print("|cFF34D4E8[SUI M+ Timer]|r Demo mode disabled.")
end
...
-- Slash Command
---------------------------------------------------------------------------
SLASH_SUIMPLUSTIMER1 = "/smpt"
SlashCmdList["SUIMPLUSTIMER"] = function(msg)
    ...
    elseif cmd == "hide" then
        ...
        print("|cFF34D4E8[SUI M+ Timer]|r Commands:")
...
-- Expose for skinning
---------------------------------------------------------------------------
_G.SuaviUI_MPlusTimer = MPlusTimer
