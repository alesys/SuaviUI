...
local function GetSettings()
    ...
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.tooltip then
        cachedSettings = SUICore.db.profile.tooltip
...
local function GetTooltipContext(owner)
    ...
    if owner.unit or                            -- Standard unit attribute
       ...
       strmatch(name, "Suavi.*Frame") then     -- SuaviUI unit frames
        return "frames"
    end
...
---------------------------------------------------------------------------
_G.SuaviUI_RefreshTooltips = function()
    InvalidateCache()
