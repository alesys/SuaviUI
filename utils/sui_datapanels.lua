...
function Datapanels:CreatePanel(panelID, config)
    if self.activePanels[panelID] then
        print("|cffff0000SuaviUI:|r Panel '" .. panelID .. "' already exists!")
        return self.activePanels[panelID]
    end
    
    -- Create panel frame
    local panel = CreateFrame("Frame", "SUI_Datapanel_" .. panelID, UIParent)
...
function Datapanels:SetupDragging(panel)
    ...
    panel:SetScript("OnDragStop", function(self)
        ...
        local db = SUICore.db.profile.suiDatatexts
...
function Datapanels:UpdateSlots(panel)
    ...
    for i = 1, numSlots do
        ...
        if datatextID and SUICore.Datatexts then
            SUICore.Datatexts:AttachToSlot(slot, datatextID, panel.config)
        else
...
_G.SuaviUI_RefreshDatapanels = function()
...
-- Debug slash command
SLASH_SUIDATAPANELS1 = "/suidp"
SlashCmdList["SUIDATAPANELS"] = function(msg)
    if msg == "show" then
        local count = 0
        ...
        print(string.format("|cff00ff00SuaviUI:|r Total panels: %d", count))
    elseif msg == "refresh" then
        Datapanels:RefreshAll()
        print("|cff00ff00SuaviUI:|r Refreshed all datapanels")
    else
        print("|cff00ff00SuaviUI Datapanels Commands:|r")
        print("/suidp show - List all panels and their status")
        print("/suidp refresh - Refresh all panels")
    end
end
