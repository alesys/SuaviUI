...
---------------------------------------------------------------------------
local SUI_UF = {}
ns.SUI_UnitFrames = SUI_UF

-- Frame references
SUI_UF.frames = {}
SUI_UF.castbars = {}
SUI_UF.previewMode = {}
SUI_UF.auraPreviewMode = {}  -- Tracks buff/debuff preview state (keyed by "unitKey_buff" or "unitKey_debuff")
...
local function GetDB()
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.suiUnitFrames then
...
local function GetHealthBarColor(unit, settings)
    ...
    -- Get global settings from MAIN profile (not suiUnitFrames sub-table)
    local general = GetGeneralSettings()
...
local function CreateBossFrame(unit, frameKey, bossIndex)
    ...
    -- Create secure button with unique name (SUI_Boss1, SUI_Boss2, etc.)
    local frameName = "SUI_Boss" .. bossIndex
    ...
    frame:HookScript("OnShow", function(self)
        ...
        if SUI_UF.previewMode[bossKey] then return end
        UpdateFrame(self)
    end)
...
local function ForceUpdateToT()
    local totFrame = SUI_UF.frames and SUI_UF.frames.targettarget
...
local function CreateUnitFrame(unit, unitKey)
    ...
    local frameName = "SUI_" .. unitKey:gsub("^%l", string.upper)
    local frame = CreateFrame("Button", frameName, UIParent, "SecureUnitButtonTemplate, BackdropTemplate, PingableUnitFrameTemplate")
...
local function CreateCastbar(unitFrame, unit, unitKey)
    ...
    if SUI_Castbar then
        return SUI_Castbar:CreateCastbar(unitFrame, unit, unitKey)
    end
    return nil
end
...
local function CreateBossCastbar(unitFrame, unit, bossIndex)
    -- Delegate to castbar module
    if SUI_Castbar then
        return SUI_Castbar:CreateBossCastbar(unitFrame, unit, bossIndex)
    end
    return nil
end
...
---------------------------------------------------------------------------
function SUI_UF:ShowPreview(unitKey)
        ...
        for i = 1, 5 do
            ...
            if frame then
                ...
                if settings and settings.castbar and settings.castbar.previewMode then
                    ...
                    if castbar and SUI_Castbar then
                        -- Trigger the castbar to show its preview
                        SUI_Castbar:RefreshBossCastbar(castbar, bossKey, settings.castbar, frame)
                    end
                end
...
---------------------------------------------------------------------------
function SUI_UF:ShowAuraPreview(unitKey, auraType)
...
function SUI_UF:ShowAuraPreviewForFrame(frame, unitKey, auraType)
    if not frame then return end
...
function SUI_UF:HideAuraPreview(unitKey, auraType)
...
function SUI_UF:HideAuraPreviewForFrame(frame, unitKey, auraType)
    if not frame then return end
...
---------------------------------------------------------------------------
function SUI_UF:RefreshFrame(unitKey)
    ...
    if unitKey == "boss" then
        ...
        if SUICore and SUICore.GetHUDFrameLevel then
            bossFrameLevel = SUICore:GetHUDFrameLevel(bossLayerPriority)
        end
    ...
    if unitKey == "player" and settings.indicators then
        ...
        if frame.indicatorFrame then
            ...
            if SUICore and SUICore.GetHUDFrameLevel then
                local indicatorLevel = SUICore:GetHUDFrameLevel(indicatorPriority)
    ...
    if castSettings and castSettings.enabled then
        if castbar and SUI_Castbar and SUI_Castbar.RefreshCastbar then
            -- Castbar exists - refresh it
            SUI_Castbar:RefreshCastbar(castbar, unitKey, castSettings, frame)
        elseif not castbar and SUI_Castbar and SUI_Castbar.CreateCastbar then
            -- Castbar doesn't exist but is now enabled - create it
            self.castbars[unitKey] = SUI_Castbar:CreateCastbar(frame, unitKey, unitKey)
        end
    end
end

function SUI_UF:RefreshAll()
...
---------------------------------------------------------------------------
SUI_UF.editModeActive = false

-- Register slider references for real-time sync during edit mode
function SUI_UF:RegisterEditModeSliders(unitKey, xSlider, ySlider)
...
function SUI_UF:NotifyPositionChanged(unitKey, offsetX, offsetY)
...
local function CreateNudgeButton(parent, direction, deltaX, deltaY, unitKey)
    ...
    btn:SetScript("OnClick", function()
        ...
        if settings then
            ...
            SUI_UF:NotifyPositionChanged(settingsKey, settings.offsetX, settings.offsetY)
        end
    end)
...
function SUI_UF:EnableEditMode()
    ...
    if not self.exitEditModeBtn then
        local exitBtn = CreateFrame("Button", "SUI_ExitEditModeBtn", UIParent, "BackdropTemplate")
    ...
    for unitKey, frame in pairs(self.frames) do
        ...
        frame:SetScript("OnMouseDown", function(self, button)
            if button == "LeftButton" and SUI_UF.editModeActive then
                local SUICore = _G.SuaviUI and _G.SuaviUI.SUICore
                if SUICore and SUICore.SelectEditModeElement then
                    SUICore:SelectEditModeElement("unitframe", self._editModeUnitKey)
                end
            end
        end)

        frame:SetScript("OnDragStart", function(self)
            if SUI_UF.editModeActive then
                ...
                self:SetScript("OnUpdate", function(self)
                    ...
                    if selfX and selfY and parentX and parentY then
                        ...
                        if settings then
                            ...
                            SUI_UF:NotifyPositionChanged(settingsKey, offsetX, offsetY)
                        end
                    end
                end)
            end
        ...
        frame:SetScript("OnKeyDown", function(self, key)
            if not SUI_UF.editModeActive then return end
...
function SUI_UF:DisableEditMode()
    ...
    local SUICore = _G.SuaviUI and _G.SuaviUI.SUICore
    if SUICore and SUICore.ClearEditModeSelection then
        SUICore:ClearEditModeSelection()
    end
...
function SUI_UF:HideBlizzardFrames()
    local db = GetDB()
    ...
    -- Hide Focus frame visuals (always hide Blizzard focus frame when SUI unit frames are enabled)
    ...
    if db.boss and db.boss.enabled then
        ...
        if BossTargetFrameContainer and not BossTargetFrameContainer._suiEditModeFixed then
            ...
            BossTargetFrameContainer._suiEditModeFixed = true
        end
    end
end
...
function SUI_UF:Initialize()
    ...
    if SUI_Castbar then
        ...
        SUI_Castbar:SetUnitFramesModule(self)
        SUI_Castbar.castbars = self.castbars
    end
...
-- Hide Blizzard's selection frames when SUI frames are enabled
...
---------------------------------------------------------------------------
function SUI_UF:RegisterWithClique()
...
_G.SuaviUI_RefreshAuras = function(unitKey)
    if unitKey then
        ...
        if unitKey == "boss" then
            ...
            local frame = SUI_UF.frames[unitKey]
            if frame then
                lastAuraUpdate[unitKey] = 0
                UpdateAuras(frame)
            end
...
_G.SuaviUI_ShowUnitFramePreview = function(unitKey)
    SUI_UF:ShowPreview(unitKey)
end

_G.SuaviUI_HideUnitFramePreview = function(unitKey)
    SUI_UF:HidePreview(unitKey)
end

_G.SuaviUI_ShowAuraPreview = function(unitKey, auraType)
    SUI_UF:ShowAuraPreview(unitKey, auraType)
end

_G.SuaviUI_HideAuraPreview = function(unitKey, auraType)
    SUI_UF:HideAuraPreview(unitKey, auraType)
end

_G.SuaviUI_ToggleUnitFrameEditMode = function()
    SUI_UF:ToggleEditMode()
end

-- Register slider references for real-time sync during edit mode
_G.SuaviUI_RegisterEditModeSliders = function(unitKey, xSlider, ySlider)
    SUI_UF:RegisterEditModeSliders(unitKey, xSlider, ySlider)
end

-- Global references for external access
_G.SuaviUI_UnitFrames = SUI_UF.frames
_G.SuaviUI_Castbars = SUI_UF.castbars

-- Castbar functions are now in SUI_Castbar.lua module
...
_G.SuaviUI_UpdateAnchoredUnitFrames = function()
    ...
    if not IsAnchorFree(playerAnchorType) and SUI_UF.frames.player then
        ...
        if anchorFrame and anchorFrame:IsShown() then
            ...
            if anchor then
                local frame = SUI_UF.frames.player
    ...
    if not IsAnchorFree(targetAnchorType) and SUI_UF.frames.target then
        ...
        if anchorFrame and anchorFrame:IsShown() then
            ...
            if anchor then
                local frame = SUI_UF.frames.target
                local frameWidth = frame:GetWidth()
                local frameHeight = frame:GetHeight()
...
_G.SuaviUI_UpdateCDMAnchoredUnitFrames = _G.SuaviUI_UpdateAnchoredUnitFrames
...
_G.SuaviUI_UpdateLockedCastbarToEssential = function(forceUpdate)
    local db = GetDB()
    ...
    if _G.SuaviUI_RefreshCastbar then
        _G.SuaviUI_RefreshCastbar("player")
    end
end

-- Global callback for NCDM to update castbar anchored to Utility
-- Width is now controlled by dual anchors, so just refresh the castbar
_G.SuaviUI_UpdateLockedCastbarToUtility = function(forceUpdate)
    local db = GetDB()
    if not db or not db.player then return end
...
_G.SuaviUI_UpdateLockedCastbarToFrame = function()
    local db = GetDB()
    if not db or not db.player then return end
