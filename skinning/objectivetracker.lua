...
-- Applies SUI color scheme with dynamic content-height backdrop
---------------------------------------------------------------------------
...
-- Style completion checkmark with SUI color
...
-- Sync SUI max height with Blizzard's editModeHeight so truncation matches backdrop
...
local function UpdateBackdropAnchors()
    ...
    TrackerFrame.suiBackdrop:ClearAllPoints()
    TrackerFrame.suiBackdrop:SetPoint("TOPLEFT", TrackerFrame, "TOPLEFT", -15, 0)
    TrackerFrame.suiBackdrop:SetPoint("TOPRIGHT", TrackerFrame, "TOPRIGHT", 10, 0)
    ...
    if bottomModule then
        ...
        if contentHeight > maxHeight then
            ...
            TrackerFrame.suiBackdrop:SetHeight(maxHeight)
        else
            -- Content fits, anchor to bottommost module
            TrackerFrame.suiBackdrop:SetPoint("BOTTOM", bottomModule, "BOTTOM", 0, -15)
        end
        TrackerFrame.suiBackdrop:Show()
    else
        -- No visible modules, hide backdrop
        TrackerFrame.suiBackdrop:Hide()
    end
end
...
local function ApplyQUIBackdrop(trackerFrame, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if trackerFrame.SetBackgroundAlpha and not trackerFrame.quiBackgroundHooked then
        hooksecurefunc(trackerFrame, "SetBackgroundAlpha", function(self, alpha)
            ...
            if self.suiBackdrop then
                ...
                self.suiBackdrop:SetBackdropColor(currBgR, currBgG, currBgB, alpha)
            end
        end)
    ...
    -- Create SUI backdrop (anchors will be set by UpdateBackdropAnchors)
    if not trackerFrame.suiBackdrop then
        trackerFrame.suiBackdrop = CreateFrame("Frame", nil, trackerFrame, "BackdropTemplate")
        trackerFrame.suiBackdrop:SetFrameLevel(math.max(trackerFrame:GetFrameLevel() - 1, 0))
        trackerFrame.suiBackdrop:EnableMouse(false)
    end

    local settings = GetSettings()
    local hideBorder = settings and settings.hideObjectiveTrackerBorder

    trackerFrame.suiBackdrop:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
        edgeFile = "Interface\\Buttons\\WHITE8x8",
        edgeSize = hideBorder and 0 or 1,
        insets = { left = 1, right = 1, top = 1, bottom = 1 }
    })
    trackerFrame.suiBackdrop:SetBackdropColor(bgr, bgg, bgb, opacity)
    if hideBorder then
        trackerFrame.suiBackdrop:SetBackdropBorderColor(0, 0, 0, 0)
    else
        trackerFrame.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    end
...
local function GetFontPath()
    local SUI = _G.SuaviUI
    return SUI and SUI.GetGlobalFont and SUI:GetGlobalFont() or STANDARD_TEXT_FONT
end
...
local function SkinObjectiveTracker()
    ...
    -- Apply SUI backdrop with our colors/opacity
    ...
    if manager and manager.SetOpacity and not manager.quiOpacityHooked then
        hooksecurefunc(manager, "SetOpacity", function(self, opacityPercent)
            ...
            local _, _, _, _, currBgR, currBgG, currBgB = GetColors()
            if TrackerFrame.suiBackdrop then
                TrackerFrame.suiBackdrop:SetBackdropColor(currBgR, currBgG, currBgB, alpha)
            end
        end)
...
_G.SuaviUI_RefreshObjectiveTracker = RefreshObjectiveTracker
_G.SuaviUI_RefreshObjectiveTrackerColors = RefreshObjectiveTracker
