...
local function IsNudgeTargetFrameName(frameName)
    ...
    if SUICore.viewers then
        for _, viewerName in ipairs(SUICore.viewers) do
...
local function CreateNudgeUI()
    ...
    local function CreateArrowButton(parent, direction, x, yFromTop)
        ...
        button:SetScript("OnClick", function()
            SUICore:NudgeSelectedViewer(direction)
    ...
    amountSlider:SetScript("OnValueChanged", function(self, value)
        ...
        SUICore.db.profile.nudgeAmount = value
    ...
    local function ViewerDropdown_Initialize(self, level)
        ...
        if SUICore.viewers then
            for _, viewerName in ipairs(SUICore.viewers) do
                ...
                info.func = function()
                    SUICore:SelectViewer(viewerName)
                    UIDropDownMenu_SetText(viewerDropdown, displayName)
                    CloseDropDownMenus()
                end
                info.checked = (SUICore.selectedViewer == viewerName)
        ...
        for frameName, label in pairs(UNIT_ANCHOR_FRAMES) do
            ...
            info.func = function()
                SUICore:SelectViewer(frameName)
                UIDropDownMenu_SetText(viewerDropdown, displayName)
                CloseDropDownMenus()
            end
            info.checked = (SUICore.selectedViewer == frameName)
            UIDropDownMenu_AddButton(info, level)
        end
    end
    ...
    function NudgeFrame:UpdateInfo()
        local viewerName = SUICore.selectedViewer
    ...
    function NudgeFrame:UpdateAmountSlider()
        local amount = SUICore.db.profile.nudgeAmount or 1
...
local function CreateMinimapNudgeButton(parent, direction)
    ...
    btn:SetScript("OnClick", function()
        SUICore:SelectEditModeElement("minimap", "minimap")
        SUICore:NudgeMinimap(direction)
    end)
...
-- Create overlay for the SUI minimap
...
function SUICore:ShowViewerOverlays()
...
function SUICore:HideViewerOverlays()
    for _, viewerName in ipairs(CDM_VIEWERS) do
        local overlay = viewerOverlays[viewerName]
        if overlay then
            overlay:Hide()
...
function SUICore:ShowBlizzardFrameOverlays()
...
function SUICore:HideBlizzardFrameOverlays()
    for _, frameInfo in ipairs(BLIZZARD_EDITMODE_FRAMES) do
...
function SUICore:ShowMinimapOverlay()
    ...
    if minimapOverlay then
        ...
        minimapOverlay:SetScript("OnMouseUp", function(self, button)
            ...
            -- Save position to DB
            local settings = SUICore.db and SUICore.db.profile and SUICore.db.profile.minimap
            if settings then
...
function SUICore:HideMinimapOverlay()
    if minimapOverlay then
        minimapOverlay:Hide()
...
function SUICore:EnableClickDetection()
    clickDetector:Show()
                    ...
                    if frame and frame ~= WorldFrame then
                        ...
                        if not frameName and frame:GetParent() then
                            ...
                            if IsNudgeTargetFrameName(parentName) then
                                if lastClickedFrame ~= frame then
                                    lastClickedFrame = frame
                                    SUICore:SelectViewer(parentName)
                                end
                                return
                            end
                        end
...
function SUICore:DisableClickDetection()
    clickDetector:Hide()
...
function SUICore:SelectViewer(viewerName)
...
function SUICore:NudgeSelectedViewer(direction)
...
-- Nudge the minimap
function SUICore:NudgeMinimap(direction)
...
local function SetupEditModeHooks()
    ...
    hooksecurefunc(EditModeManagerFrame, "EnterEditMode", function()
        ...
        if SUICore.nudgeFrame then
            SUICore.nudgeFrame:UpdateVisibility()
        end
        SUICore:EnableClickDetection()
...
-- Add nudgeamount
local oldOnInitialize = SUICore.OnInitialize
