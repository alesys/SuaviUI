...
function BarMixin:Init(config, parent, frameLevel)
    ...
    -- Use Blizzard's built-in statusbar texture as the default
    ...
    self.Mask:SetTexture([[Interface\AddOns\SuaviUI\assets\textures\white.png]])
...
function BarMixin:InitCooldownManagerWidthHook(layoutName)
    ...
    self._SUI_Essential_Utility_hook_widthMode = data.widthMode
...
function BarMixin:GetData(layoutName)
    ...
    local db = dbRoot[self.config.dbName]
    return db and db[layoutName]
end
...
function BarMixin:UpdateDisplay(layoutName, force)
    ...
    if RB.fragmentedPowerTypes[resource] then
...
function BarMixin:GetPoint(layoutName)
    ...
    if not data then
        return defaults.point or "CENTER",
            RB.resolveRelativeFrames(defaults.relativeFrame or "UIParent"),
            defaults.relativePoint or "CENTER",
            defaults.x or 0,
            defaults.y or 0
    end

    local x = (data.x ~= nil) and data.x or defaults.x
    local y = (data.y ~= nil) and data.y or defaults.y
    ...
    local resolvedRelativeFrame = RB.resolveRelativeFrames(relativeFrame) or UIParent
    ...
    if self.Frame == resolvedRelativeFrame or self.Frame == select(2, resolvedRelativeFrame:GetPoint(1)) then
        ...
        RB.prettyPrint(L["RELATIVE_FRAME_CYCLIC_WARNING"])
    end

    local uiWidth, uiHeight = UIParent:GetWidth() / 2, UIParent:GetHeight() / 2
    return point, resolvedRelativeFrame, relativePoint, RB.clamp(x, uiWidth * -1, uiWidth), RB.clamp(y, uiHeight * -1, uiHeight)
end
...
function BarMixin:GetSize(layoutName, data)
    ...
    if data.widthMode ~= nil and data.widthMode ~= RB.WIDTH_MODE.MANUAL then
        ...
        width = (data.width ~= nil) and data.width or defaults.width
    end

    local height = (data.height ~= nil) and data.height or defaults.height
    local scale = (data.scale ~= nil) and RB.rounded(data.scale, 2) or RB.rounded(defaults.scale or 1, 2)
...
function BarMixin:ApplyMaskAndBorderSettings(layoutName, data)
    ...
    if style.type == "fixed" then
        ...
        local ppScale = RB.getPixelPerfectScale()
        local pThickness = math.max(RB.rounded(thickness), 1) * ppScale
...
function BarMixin:ApplyBackgroundSettings(layoutName, data)
    ...
    local bgConfig = RB.backgroundStyles[bgStyleName]
...
function BarMixin:ApplyForegroundSettings(layoutName, data)
    ...
    -- Try to fetch from LSM, but always have a fallback to Blizzard's built-in gradient
    local fgTexture = LSM:Fetch(LSM.MediaType.STATUSBAR, fgStyleName)
    if not fgTexture or fgTexture == "" then
        fgTexture = [[Interface\TargetingFrame\UI-StatusBar]]
    end
...
function BarMixin:UpdateTicksLayout(layoutName, data)
    ...
    elseif resource == "TIP_OF_THE_SPEAR" then
        max = RB.TipOfTheSpear.TIP_MAX_STACKS
    elseif resource == "WHIRLWIND" then
        max = RB.Whirlwind.IW_MAX_STACKS
    ...
    -- Show 4 ticks for edit mode preview if spec doesn't support it
    if LEM:IsInEditMode() and data.showTicks == true and type(resource) ~= "string" and RB.tickedPowerTypes[resource] == nil then
        max = 5
        resource = Enum.PowerType.ComboPoints
    end

    self.Ticks = self.Ticks or {}
    if data.showTicks == false or not RB.tickedPowerTypes[resource] then
    ...
    for i = 1, needed do
        ...
        if self.StatusBar:GetOrientation() == "VERTICAL" then
            ...
            local snappedX = RB.rounded(rawX / ppScale) * ppScale
...
function BarMixin:UpdateFragmentedPowerDisplay(layoutName, data, maxPower)
    ...
    if resource == Enum.PowerType.ComboPoints then
        ...
        local overchargedCpColor = RB:GetOverrideResourceColor("OVERCHARGED_COMBO_POINTS") or color
