...
-- Version is read dynamically from the TOC file
local ADDON_VERSION = C_AddOns.GetAddOnMetadata(ADDON_NAME, "Version") or "0.0.0"
...
SUICore.__pendingReload = false
SUICore.__reloadEventFrame = nil

-- Safe reload function - queues if in combat, reloads immediately if not
function SUICore:SafeReload()
    if InCombatLockdown() then
        if not self.__pendingReload then
            self.__pendingReload = true
            print("|cFF30D1FFSuavi UI:|r Reload queued - will execute when combat ends.")
            ...
            if not self.__reloadEventFrame then
                ...
                self.__reloadEventFrame:SetScript("OnEvent", function(frame, event)
                    if event == "PLAYER_REGEN_ENABLED" and SUICore.__pendingReload then
                        SUICore.__pendingReload = false
                        -- Show popup with reload button (user click = allowed)
                        SUICore:ShowReloadPopup()
                    end
                end)
            end
        end
...
function SUICore:ShowReloadPopup()
    -- Use SuaviUI's existing confirmation dialog
    if SuaviUI and SuaviUI.GUI and SuaviUI.GUI.ShowConfirmation then
        ...
        print("|cFF30D1FFSuavi UI:|r Combat ended. Type /reload to reload.")
    end
end

-- Global safe reload function on SuaviUI object
function SUI:SafeReload()
    if self.SUICore then
        self.SUICore:SafeReload()
    else
        -- Fallback if SUICore not loaded
        if InCombatLockdown() then
            print("|cFF30D1FFSuavi UI:|r Cannot reload during combat.")
        else
            ReloadUI()
        end
    end
...
function SUICore:ExportProfileToString()
...
function SUICore:GetHUDFrameLevel(priority)
...
function SUICore.SafeSetBackdrop(frame, backdropInfo, borderColor)
    ...
    if not hasValidSize then
        ...
        SUICore.__pendingBackdrops = SUICore.__pendingBackdrops or {}
        SUICore.__pendingBackdrops[frame] = true

        -- Set up deferred processing via OnUpdate (for when dimensions become valid)
        if not SUICore.__backdropUpdateFrame then
            ...
            updateFrame:SetScript("OnUpdate", function(self, delta)
                ...
                local processed = {}
                for pendingFrame in pairs(SUICore.__pendingBackdrops or {}) do
                ...
                for _, pf in ipairs(processed) do
                    SUICore.__pendingBackdrops[pf] = nil
                end

                -- Stop OnUpdate if no more pending
                local hasAny = false
                for _ in pairs(SUICore.__pendingBackdrops or {}) do
                    hasAny = true
                    break
                end
                if not hasAny then
            ...
            SUICore.__backdropUpdateFrame = updateFrame
        end
        SUICore.__backdropUpdateFrame:Show()
        return false
    end

    -- If in combat, defer backdrop setup to avoid secret value errors
    if InCombatLockdown() then
        local alreadyPending = SUICore.__pendingBackdrops and SUICore.__pendingBackdrops[frame]
        if not alreadyPending then
            frame.__quiBackdropPending = backdropInfo
            frame.__quiBackdropBorderColor = borderColor

            if not SUICore.__backdropEventFrame then
                ...
                eventFrame:SetScript("OnEvent", function(self)
                    ...
                    SUICore.__pendingBackdrops = {}
                end)
                SUICore.__backdropEventFrame = eventFrame
            end

            SUICore.__pendingBackdrops = SUICore.__pendingBackdrops or {}
            SUICore.__pendingBackdrops[frame] = true
            SUICore.__backdropEventFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
        end
        return false
    end
...
SUICore.viewers = {
    ...
    -- "BuffIconCooldownViewer",   -- Handled by sui_buffbar.lua
}
...
local defaults = {
    profile = {
        ...
        general = {
            ...
            texture = "Suavitex v3",  -- Default bar texture
            darkMode = false,
            ...
            addSuaviUIButton = false,  -- Add Suavi UI button to ESC menu (opt-in)
        ...
        -- SUI New Cooldown Display Manager (NCDM)
        ...
        viewers = {
            ...
            -- BuffIconCooldownViewer removed - now handled by sui_buffbar.lua
        ...
        -- SUI Unit Frames (New Implementation)
        suiUnitFrames = {
            enabled = true,
        ...
        combatTimer = {
            ...
            font = "Suavi",       -- Font name (from LibSharedMedia)
...
function SUICore:OnInitialize()
    ...
    -- Migrate old "qui" prefixed database keys to "sui" (SuaviUI rebranding)
    ...
    if profile.quiDatatexts and not profile.suiDatatexts then
...
function SUICore:OnProfileChanged(event, db, profileKey)
    ...
    local function ApplyUIScale(scale)
        if InCombatLockdown() then
            SUICore._pendingUIScale = scale
            if not SUICore._scaleRegenFrame then
                SUICore._scaleRegenFrame = CreateFrame("Frame")
                SUICore._scaleRegenFrame:SetScript("OnEvent", function(self)
                    self:UnregisterEvent("PLAYER_REGEN_ENABLED")
                    if SUICore._pendingUIScale and not InCombatLockdown() then
                        pcall(function() UIParent:SetScale(SUICore._pendingUIScale) end)
                        SUICore._pendingUIScale = nil
                        if SUICore.UIMult then
                            SUICore:UIMult()
                        end
                    end
                end)
            end
            SUICore._scaleRegenFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
        else
    ...
    if SUICore.Minimap then
        -- Small delay to ensure profile data is fully loaded
        C_Timer.After(0.1, function()
            if SUICore.Minimap.Refresh then
...
function SUICore:SelectEditModeElement(elementType, elementKey)
...
function SUICore:ClearEditModeSelection()
    self:HideCurrentSelectionArrows()
...
function SUICore:HideCurrentSelectionArrows()
    ...
    if sel.selectedType == "unitframe" then
        ...
        if ns.SUI_UnitFrames and ns.SUI_UnitFrames.frames then
            local frame = ns.SUI_UnitFrames.frames[sel.selectedKey]
...
function SUICore:ShowSelectionArrows(elementType, elementKey)
    if elementType == "unitframe" then
        if ns.SUI_UnitFrames and ns.SUI_UnitFrames.frames then
            local frame = ns.SUI_UnitFrames.frames[elementKey]
            if frame and frame.editOverlay then
...
function SUICore:ShowNudgeButtons(overlay)
...
function SUICore:HideNudgeButtons(overlay)
    if not overlay then return end
...
function SUICore:OnEnable()
    ...
    C_Timer.After(1.0, function()
        ...
        if _G.SuaviUI_RefreshUIHider then
            _G.SuaviUI_RefreshUIHider()
        end
...
function SUICore:OpenConfig()
    -- Open the new custom GUI instead of AceConfig
    if SuaviUI and SuaviUI.GUI then
        SuaviUI.GUI:Toggle()
    end
end

function SUICore:CreateMinimapButton()
...
function SUICore:SkinIcon(icon, settings)
...
function SUICore:ApplyViewerLayout(viewer)
    if not viewer or not viewer.GetName then return end
    local name     = viewer:GetName()
    local settings = self.db.profile.viewers[name]
    if not settings or not settings.enabled then return end
...
function SUICore:RescanViewer(viewer)
    if not viewer or not viewer.GetName then return end
    local name     = viewer:GetName()
    local settings = self.db.profile.viewers[name]
    if not settings or not settings.enabled then return end
...
function SUICore:ApplyViewerSkin(viewer)
    if not viewer or not viewer.GetName then return end
    local name     = viewer:GetName()
    local settings = self.db.profile.viewers[name]
    if not settings or not settings.enabled then return end
...
function SUICore:ProcessPendingIcons()
...
function SUICore:ForceReskinAllViewers()
    for _, name in ipairs(self.viewers) do
        local viewer = _G[name]
        if viewer then
            local container = viewer.viewerFrame or viewer
        ...
        pcall(hooksecurefunc, EditModeManagerFrame, "ExitEditMode", function()
            C_Timer.After(0.1, function()
                ...
                C_Timer.After(0.15, function()
                    for _, barName in ipairs({"SuaviUIPrimaryPowerBar", "SuaviUISecondaryPowerBar"}) do
...
function SUICore:ProcessPendingBackdrops()
...
function SUI:GetGlobalFont()
    local LSM = LibStub("LibSharedMedia-3.0")
    local fontName = "Suavi"  -- Default fallback
...
function SUI:GetGlobalTexture()
    local LSM = LibStub("LibSharedMedia-3.0")
    -- For now, return Suavi texture. Will be configurable in General Tab (Feature #3)
    local textureName = "Suavi"
    return LSM:Fetch("statusbar", textureName) or "Interface\\AddOns\\SuaviUI\\assets\\Suavi"
end

function SUI:GetSkinColor()
    local db = SUI.db and SUI.db.profile
...
function SUICore:SafeSetFont(fontString, fontPath, size, flags)
...
local function GetGlobalFontPath()
    if not SUICore.db or not SUICore.db.profile or not SUICore.db.profile.general then
        return SUAVI_FONT_PATH
    end
    local fontName = SUICore.db.profile.general.font or "Suavi"
    local fontPath = LSM:Fetch("font", fontName)
    return fontPath or SUAVI_FONT_PATH
end
...
local function ScheduleGlobalFontApply()
    ...
    C_Timer.After(0.05, function()
        ...
        if SUICore.ApplyGlobalFont then
            SUICore:ApplyGlobalFont()
        end
    end)
end

function SUICore:ApplyGlobalFont()
    ...
    if not globalFontHooksInitialized then
        ...
        chatFontEventFrame:SetScript("OnEvent", function()
            if not SUICore.db or not SUICore.db.profile then return end
            if not SUICore.db.profile.general.applyGlobalFontToBlizzard then return end
            C_Timer.After(0.05, function()
                local fp = GetGlobalFontPath()
