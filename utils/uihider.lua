...
local function GetSettings()
    ...
    if not SUICore.db.profile.uiHider then
        SUICore.db.profile.uiHider = {
    ...
    local uiHider = SUICore.db.profile.uiHider
...
local function ApplyHideSettings()
    ...
    if BuffFrame and BuffFrame.CollapseAndExpandButton then
        ...
        if settings.hideBuffCollapseButton then
            ...
            if not btn._SUI_AlphaHooked then
                btn._SUI_AlphaHooked = true
    ...
    if TalkingHeadFrame then
        ...
        if settings.hideTalkingHead then
            ...
            if not TalkingHeadFrame._SUI_MouseManaged then
                TalkingHeadFrame._SUI_MouseManaged = true
    ...
    if StatusTrackingBarManager then
        ...
        -- else: Do nothing - let other addons/Blizzard manage the bar when SUI isn't hiding
    end
...
-- Initialize
-- Note: ApplyHideSettings is now called from suicore_main.lua OnEnable()
...
_G.SuaviUI_RefreshUIHider = function()
    ApplyHideSettings()
end
