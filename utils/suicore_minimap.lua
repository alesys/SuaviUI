...
SUICore.Minimap = Minimap_Module
...
-- Datatext panel (3-slot architecture using SUICore.Datatexts registry)
local datatextFrame
...
local function GetSettings()
    ...
    cachedSettings = SUICore.db.profile.minimap
...
local function CreateBackdrop()
    ...
    backdropFrame = CreateFrame("Frame", "sui_MinimapBackdrop", Minimap)
...
local function GetDatatextSettings()
    ...
    return SUICore.db.profile.datatext
end
...
local function CreateDatatextPanel()
    ...
    datatextFrame = CreateFrame("Frame", "sui_DatatextPanel", UIParent)
...
local function RefreshDatatextSlots()
    ...
    if not SUICore or not SUICore.Datatexts then return end
    ...
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.general then
        ...
        generalFont = general.font or "Suavi"
    ...
    for i, slot in ipairs(datatextFrame.slots) do
        ...
        if slot.datatextInstance then
            SUICore.Datatexts:DetachFromSlot(slot)
        end

        -- Apply font to ALL slots (prevents "Font not set" error on empty slots)
        SUICore:SafeSetFont(slot.text, fontPath, fontSize, generalOutline)
        ...
        if datatextID and datatextID ~= "" then
            ...
            -- Attach datatext
            SUICore.Datatexts:AttachToSlot(slot, datatextID, dtSettings)
        else
            -- Empty slot: hide
            slot:Hide()
            slot.text:SetText("")
...
local function UpdateClock()
    ...
    SUICore:SafeSetFont(clockText, fontPath, clockConfig.fontSize, flags)
...
local function UpdateCoords()
    ...
    SUICore:SafeSetFont(coordsText, fontPath, coordsConfig.fontSize, flags)
...
local function UpdateZoneText()
    ...
    local fontPath = LSM:Fetch("font", generalFont) or "Fonts\\FRIZQT__.TTF"
    SUICore:SafeSetFont(zoneTextFont, fontPath, zoneConfig.fontSize, flags)
...
function SUICore:EnableMinimapEditMode()
    local settings = GetSettings()
...
function SUICore:DisableMinimapEditMode()
    local settings = GetSettings()
    if not settings then return end
...
function Minimap_Module:Refresh()
    ...
    if not settings.enabled then
        StopUpdateTickers()
        -- Hide SUI customizations but keep minimap visible
        if minimapBackdrop then
            minimapBackdrop:Hide()
        end
