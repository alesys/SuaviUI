...
-- Scan mode toggle (explicit /suiscan)
...
local function GetDB()
    if SUI and SUI.db and SUI.db.global then
        ...
        if SUI.db.global.spellScanner.autoScan ~= nil then
            SpellScanner.autoScan = SUI.db.global.spellScanner.autoScan
        end
        return SUI.db.global.spellScanner
    end
    return nil
end
    ...
    if bestMatch then
        ...
        if success then
            ...
            if SpellScanner.scanMode then
                print(string.format("|cff00ff00SuaviUI:|r Scanned: %s = %.1fs",
...
-- /suiscan - Toggle scan mode
SLASH_SUISCAN1 = "/suiscan"
SlashCmdList["SUISCAN"] = function()
    local enabled = SpellScanner.ToggleScanMode()
    if enabled then
        print("|cff00ff00SuaviUI:|r Scan mode |cff00ff00ENABLED|r")
        print("|cffff8800-|r Cast abilities to scan their durations")
        print("|cffff8800-|r Type /suiscan again to stop")
    else
        print("|cff00ff00SuaviUI:|r Scan mode |cffff0000DISABLED|r")
    end
end

-- /suiscanned - List scanned spells
SLASH_SUISCANNED1 = "/suiscanned"
SlashCmdList["SUISCANNED"] = function()
    local db = GetDB()
    if not db then
        print("|cffff0000SuaviUI:|r Database not available")
        return
    end

    print("|cff00ff00SuaviUI Scanned Spells:|r")
    local spellCount = 0
    ...
    print("|cff00ff00SuaviUI Scanned Items:|r")
    local itemCount = 0
...
-- /suiclearspell <spellID> - Remove a scanned spell
SLASH_SUICLEARSPELL1 = "/suiclearspell"
SlashCmdList["SUICLEARSPELL"] = function(msg)
    local spellID = tonumber(msg:trim())
    if not spellID then
        print("|cffff0000SuaviUI:|r Usage: /suiclearspell <spellID>")
        return
    end
    ...
    if db and db.spells and db.spells[spellID] then
        ...
        print(string.format("|cff00ff00SuaviUI:|r Cleared spell: %s [%d]", name, spellID))
    else
        print(string.format("|cffff8800SuaviUI:|r Spell %d not found in scanned list", spellID))
    end
end
