--- SuaviUI Alert & Toast Skinning
--- Skins Blizzard alert frames with SUI styling and adds movers
...
-- Text color
local SUI_TEXT_COLOR = { 0.953, 0.957, 0.965, 1 }
...
--- Get theme colors from SUI skinning system
...
--- Create SUI-styled backdrop for alert frames
local function CreateAlertBackdrop(frame, xOffset1, yOffset1, xOffset2, yOffset2)
    if frame.suiBackdrop then return frame.suiBackdrop end
...
local function UpdateBackdropColors(frame)
    if not frame.suiBackdrop then return end
...
local function SkinAchievementAlert(frame)
    ...
    if frame.Unlocked then
        frame.Unlocked:SetTextColor(unpack(SUI_TEXT_COLOR))
    end
    if frame.Name then
...
local function SkinCriteriaAlert(frame)
    ...
    if frame.Unlocked then frame.Unlocked:SetTextColor(unpack(SUI_TEXT_COLOR)) end
...
local function SkinLootWonAlert(frame)
    ...
    if not frame.suiBackdrop and lootItem.Icon.quiBorder then
        local sr, sg, sb, sa, bgr, bgg, bgb, bga = GetThemeColors()
...
local function SkinLootUpgradeAlert(frame)
    ...
    -- Create backdrop
    if not frame.suiBackdrop and frame.Icon.quiBorder then
        local sr, sg, sb, sa, bgr, bgg, bgb, bga = GetThemeColors()
...
local function SkinNewRecipeLearnedAlert(frame)
    ...
    if frame.Icon then
        ...
        frame.Icon:SetPoint("LEFT", frame.suiBackdrop, 9, 0)
...
local function SkinScenarioAlert(frame)
    ...
    if frame.dungeonTexture then
        ...
        frame.dungeonTexture:ClearAllPoints()
        frame.dungeonTexture:SetPoint("LEFT", frame.suiBackdrop, 9, 0)
        frame.dungeonTexture:SetDrawLayer("OVERLAY")
...
local function SkinMiscAlert(frame)
    ...
    if frame.Icon then
        ...
        -- Keep original parent; reparenting to the border can create anchor dependency loops.
...
local function SkinBonusRollFrames()
    ...
    if moneyFrame and not moneyFrame.quiSkinned then
        ...
        backdrop:SetBackdropColor(bgr, bgg, bgb, bga)
        backdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        moneyFrame.suiBackdrop = backdrop
    ...
    if lootFrame and not lootFrame.quiSkinned then
        ...
        backdrop:SetBackdropColor(bgr, bgg, bgb, bga)
        backdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        lootFrame.suiBackdrop = backdrop
...
local function CreateAlertMover()
    ...
    if not alertHolder then
        alertHolder = CreateFrame("Frame", "SUI_AlertFrameHolder", UIParent)
        ...
        alertMover = CreateFrame("Frame", "SUI_AlertFrameMover", alertHolder, "BackdropTemplate")
...
local function CreateEventToastMover()
    ...
    if not toastHolder then
        toastHolder = CreateFrame("Frame", "SUI_EventToastHolder", UIParent)
        ...
        -- Create mover overlay
        toastMover = CreateFrame("Frame", "SUI_EventToastMover", toastHolder, "BackdropTemplate")
...
local function RefreshAlertColors()
    ...
    if BonusRollMoneyWonFrame and BonusRollMoneyWonFrame.suiBackdrop then
        BonusRollMoneyWonFrame.suiBackdrop:SetBackdropColor(bgr, bgg, bgb, bga)
        BonusRollMoneyWonFrame.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    end
    if BonusRollLootWonFrame and BonusRollLootWonFrame.suiBackdrop then
        BonusRollLootWonFrame.suiBackdrop:SetBackdropColor(bgr, bgg, bgb, bga)
        BonusRollLootWonFrame.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    end
end

-- Expose refresh function globally
_G.SuaviUI_RefreshAlertColors = RefreshAlertColors
