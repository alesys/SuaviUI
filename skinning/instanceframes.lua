...
local function StyleDropdown(dropdown, sr, sg, sb, sa, bgr, bgg, bgb, bga, width)
    ...
    -- Create backdrop
    if not dropdown.suiBackdrop then
        dropdown.suiBackdrop = CreateFrame("Frame", nil, dropdown, "BackdropTemplate")
        dropdown.suiBackdrop:SetPoint("TOPLEFT", 0, -2)
        dropdown.suiBackdrop:SetPoint("BOTTOMRIGHT", 0, 2)
        dropdown.suiBackdrop:SetFrameLevel(dropdown:GetFrameLevel())
        dropdown.suiBackdrop:EnableMouse(false)
    end

    dropdown.suiBackdrop:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
        edgeFile = "Interface\\Buttons\\WHITE8x8",
        edgeSize = 1,
        insets = { left = 1, right = 1, top = 1, bottom = 1 }
    ...
    local btnBgG = math.min(bgg + 0.07, 1)
    local btnBgB = math.min(bgb + 0.07, 1)
    dropdown.suiBackdrop:SetBackdropColor(btnBgR, btnBgG, btnBgB, 1)
    dropdown.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)

    -- Store colors for hover
    dropdown.suiSkinColor = { sr, sg, sb, sa }
...
local function StyleTab(tab, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    -- Create backdrop
    if not tab.suiBackdrop then
        tab.suiBackdrop = CreateFrame("Frame", nil, tab, "BackdropTemplate")
        tab.suiBackdrop:SetPoint("TOPLEFT", 3, -3)
        tab.suiBackdrop:SetPoint("BOTTOMRIGHT", -3, 0)
        tab.suiBackdrop:SetFrameLevel(tab:GetFrameLevel())
        tab.suiBackdrop:EnableMouse(false)
    end

    tab.suiBackdrop:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
        edgeFile = "Interface\\Buttons\\WHITE8x8",
        edgeSize = 1,
        insets = { left = 1, right = 1, top = 1, bottom = 1 }
    })
    tab.suiBackdrop:SetBackdropColor(bgr, bgg, bgb, 0.9)
    tab.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
...
local function StyleGroupFinderButton(button, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if button.icon then
        ...
        -- Add icon border
        if not button.icon.suiBackdrop then
            button.icon.suiBackdrop = CreateFrame("Frame", nil, button, "BackdropTemplate")
            button.icon.suiBackdrop:SetPoint("TOPLEFT", button.icon, -1, 1)
            button.icon.suiBackdrop:SetPoint("BOTTOMRIGHT", button.icon, 1, -1)
            button.icon.suiBackdrop:SetFrameLevel(button:GetFrameLevel())
            button.icon.suiBackdrop:EnableMouse(false)
            button.icon.suiBackdrop:SetBackdrop({
                bgFile = nil,
                edgeFile = "Interface\\Buttons\\WHITE8x8",
                edgeSize = 1,
            })
            button.icon.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        end
    end
...
-- Style close button (SUI pattern - minimal, just hide border)
...
local function SkinPVEFrame()
    ...
    -- Style close button (minimal - just hide border per SUI pattern)
    ...
    -- SUI: Tab1 at x=-3, tabs at -5px spacing
...
local function StyleDungeonIcon(icon, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if icon.Icon then
        ...
        if not icon.Icon.suiBackdrop then
            icon.Icon.suiBackdrop = CreateFrame("Frame", nil, icon, "BackdropTemplate")
            icon.Icon.suiBackdrop:SetPoint("TOPLEFT", icon.Icon, -1, 1)
            icon.Icon.suiBackdrop:SetPoint("BOTTOMRIGHT", icon.Icon, 1, -1)
            icon.Icon.suiBackdrop:SetFrameLevel(icon:GetFrameLevel())
            icon.Icon.suiBackdrop:EnableMouse(false)
            icon.Icon.suiBackdrop:SetBackdrop({
                bgFile = nil,
                edgeFile = "Interface\\Buttons\\WHITE8x8",
                edgeSize = 1,
            })
            icon.Icon.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        end
    end

    -- Store colors
    icon.suiSkinColor = { sr, sg, sb, sa }
...
local function StyleAffixIcon(affix, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if affix.Portrait then
        ...
        if not affix.Portrait.suiBackdrop then
            affix.Portrait.suiBackdrop = CreateFrame("Frame", nil, affix, "BackdropTemplate")
            affix.Portrait.suiBackdrop:SetPoint("TOPLEFT", affix.Portrait, -1, 1)
            affix.Portrait.suiBackdrop:SetPoint("BOTTOMRIGHT", affix.Portrait, 1, -1)
            affix.Portrait.suiBackdrop:SetFrameLevel(affix:GetFrameLevel())
            affix.Portrait.suiBackdrop:EnableMouse(false)
            affix.Portrait.suiBackdrop:SetBackdrop({
                bgFile = nil,
                edgeFile = "Interface\\Buttons\\WHITE8x8",
                edgeSize = 1,
            })
            affix.Portrait.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        end
    end
...
local function StylePVPActivityButton(button, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if button.Reward then
        ...
        if reward.Icon then
            ...
            if not reward.Icon.suiBackdrop then
                reward.Icon.suiBackdrop = CreateFrame("Frame", nil, reward, "BackdropTemplate")
                reward.Icon.suiBackdrop:SetPoint("TOPLEFT", reward.Icon, -1, 1)
                reward.Icon.suiBackdrop:SetPoint("BOTTOMRIGHT", reward.Icon, 1, -1)
                reward.Icon.suiBackdrop:SetFrameLevel(reward:GetFrameLevel())
                reward.Icon.suiBackdrop:EnableMouse(false)
                reward.Icon.suiBackdrop:SetBackdrop({
                    bgFile = nil,
                    edgeFile = "Interface\\Buttons\\WHITE8x8",
                    edgeSize = 1,
                })
                reward.Icon.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
            end
        end
    end
...
local function StyleSpecificBGButton(button, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    button.suiBackdrop:SetBackdropColor(btnBgR, btnBgG, btnBgB, 0.9)
    button.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    ...
    -- Add hover highlight
    button:HookScript("OnEnter", function(self)
        if self.suiBackdrop then
            self.suiBackdrop:SetBackdropBorderColor(1, 1, 1, 1)
        end
    end)
    button:HookScript("OnLeave", function(self)
        if self.suiBackdrop then
            self.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
        end
    end)
...
local function StyleConquestBar(bar, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if not bar.suiBackdrop then
        ...
        bar.suiBackdrop:SetFrameLevel(bar:GetFrameLevel())
    ...
    bar.suiBackdrop:SetBackdropColor(bgr, bgg, bgb, 0.8)
...
local function UpdateTabColors(tab, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    if not tab or not tab.suiBackdrop then return end
    tab.suiBackdrop:SetBackdropColor(bgr, bgg, bgb, 0.9)
    tab.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
end
...
local function UpdateGroupFinderButtonColors(button, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if button.icon and button.icon.suiBackdrop then
        button.icon.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    end
end
...
local function UpdatePVPActivityButtonColors(button, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    ...
    if button.Reward and button.Reward.Icon and button.Reward.Icon.suiBackdrop then
        button.Reward.Icon.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    end
end

-- Helper to update conquest bar colors
local function UpdateConquestBarColors(bar, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    if not bar or not bar.suiBackdrop then return end
    bar.suiBackdrop:SetBackdropColor(bgr, bgg, bgb, 0.8)
    bar.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
end
...
local function UpdateDungeonIconColors(icon, sr, sg, sb, sa)
    if not icon or not icon.Icon or not icon.Icon.suiBackdrop then return end
    icon.Icon.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    icon.suiSkinColor = { sr, sg, sb, sa }
end
...
local function UpdateAffixIconColors(affix, sr, sg, sb, sa)
    if not affix or not affix.Portrait or not affix.Portrait.suiBackdrop then return end
    affix.Portrait.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
end

-- Helper to update dropdown colors
local function UpdateDropdownColors(dropdown, sr, sg, sb, sa, bgr, bgg, bgb, bga)
    if not dropdown or not dropdown.suiBackdrop then return end
    local btnBgR = math.min(bgr + 0.07, 1)
    local btnBgG = math.min(bgg + 0.07, 1)
    local btnBgB = math.min(bgb + 0.07, 1)
    dropdown.suiBackdrop:SetBackdropColor(btnBgR, btnBgG, btnBgB, 1)
...
local function RefreshInstanceFramesColors()
    ...
    if PVEFrame.suiBackdrop then
        PVEFrame.suiBackdrop:SetBackdropColor(bgr, bgg, bgb, bga)
        PVEFrame.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
    end
    ...
    if LFGListFrame and LFGListFrame.quiSkinned then
        ...
        if LFGListFrame.SearchPanel then
            ...
            if LFGListFrame.SearchPanel.SearchBox and LFGListFrame.SearchPanel.SearchBox.suiBackdrop then
                LFGListFrame.SearchPanel.SearchBox.suiBackdrop:SetBackdropColor(bgr, bgg, bgb, bga)
                LFGListFrame.SearchPanel.SearchBox.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, sa)
            end
        end
        if LFGListFrame.ApplicationViewer then
...
_G.SuaviUI_RefreshInstanceFramesColors = RefreshInstanceFramesColors
