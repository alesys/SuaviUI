--- SuaviUI Loot & Roll Frames
--- Custom loot window and roll frames with SUI styling
...
-- Helper to get theme colors from SUI skin system
...
local function GetGeneralFont()
    local db = SUICore.db and SUICore.db.profile and SUICore.db.profile.general
    return (db and db.font) or "Suavi"
end

local function GetDB()
    return SUICore.db and SUICore.db.profile or {}
end
...
local function CreateLootSlot(parent, index)
    ...
    local slot = CreateFrame("Button", "SUI_LootSlot"..index, parent)
...
local function CreateLootWindow()
    ...
    local frame = CreateFrame("Frame", "SUI_LootFrame", UIParent, "BackdropTemplate")
...
local function CreateRollFrame(index)
    ...
    local frame = CreateFrame("Frame", "SUI_LootRollFrame"..index, UIParent, "BackdropTemplate")
...
local function CreateRollAnchor()
    local anchor = CreateFrame("Frame", "SUI_LootRollAnchor", UIParent, "BackdropTemplate")
...
local function SkinGroupLootHistoryFrame()
    ...
    -- Apply SUI backdrop
    if not HistoryFrame.suiBackdrop then
        HistoryFrame.suiBackdrop = CreateFrame("Frame", nil, HistoryFrame, "BackdropTemplate")
        HistoryFrame.suiBackdrop:SetAllPoints()
        HistoryFrame.suiBackdrop:SetFrameLevel(HistoryFrame:GetFrameLevel())
        HistoryFrame.suiBackdrop:SetBackdrop({
            bgFile = "Interface\\Buttons\\WHITE8x8",
            edgeFile = "Interface\\Buttons\\WHITE8x8",
            edgeSize = 1,
        })
    end
    HistoryFrame.suiBackdrop:SetBackdropColor(unpack(bgColor))
    HistoryFrame.suiBackdrop:SetBackdropBorderColor(unpack(borderColor))
    ...
    if ResizeButton then
        ...
        if not ResizeButton.suiBackdrop then
            ResizeButton.suiBackdrop = CreateFrame("Frame", nil, ResizeButton, "BackdropTemplate")
            ResizeButton.suiBackdrop:SetAllPoints()
            ResizeButton.suiBackdrop:SetBackdrop({
                bgFile = "Interface\\Buttons\\WHITE8x8",
                edgeFile = "Interface\\Buttons\\WHITE8x8",
                edgeSize = 1,
            })
            ResizeButton.suiBackdrop:SetBackdropColor(bgColor[1], bgColor[2], bgColor[3], 0.8)
            ResizeButton.suiBackdrop:SetBackdropBorderColor(unpack(borderColor))
...
function Loot:ApplyLootHistoryTheme()
    ...
    if not enabled then
        if HistoryFrame.suiBackdrop then
            HistoryFrame.suiBackdrop:Hide()
        end
        if HistoryFrame.NineSlice then
        ...
        if HistoryFrame.ResizeButton and HistoryFrame.ResizeButton.suiBackdrop then
            HistoryFrame.ResizeButton.suiBackdrop:Hide()
    ...
    -- Enabled - apply SUI skin
    if not HistoryFrame.suiBackdrop then return end

    local bgColor, borderColor, textColor = GetThemeColors()

    -- Show our backdrop, hide Blizzard's
    HistoryFrame.suiBackdrop:Show()
    ...
    if HistoryFrame.ResizeButton and HistoryFrame.ResizeButton.suiBackdrop then
        HistoryFrame.ResizeButton.suiBackdrop:Show()
        if HistoryFrame.ResizeButton.NineSlice then
            HistoryFrame.ResizeButton.NineSlice:SetAlpha(0)
        end
        HistoryFrame.ResizeButton.suiBackdrop:SetBackdropColor(bgColor[1], bgColor[2], bgColor[3], 0.8)
        HistoryFrame.ResizeButton.suiBackdrop:SetBackdropBorderColor(unpack(borderColor))
        if HistoryFrame.ResizeButton.quiText then
...
_G.SuaviUI_RefreshLootColors = function()
    if SUICore and SUICore.Loot then
        SUICore.Loot:RefreshColors()
    end
end
...
local EDIT_BORDER_COLOR = { 0.2, 0.8, 0.8, 1 }  -- Cyan/teal to match SUI style
local EDIT_BORDER_SIZE = 2

-- Create border highlight around a frame (matching SUI player frame style)
...
-- Initialize when SUICore is enabled
local initFrame = CreateFrame("Frame")
initFrame:RegisterEvent("PLAYER_LOGIN")
initFrame:SetScript("OnEvent", function(self, event)
    -- Defer initialization to let SUICore load first
