...
-- SUI skinning for ReadyCheckFrame
---------------------------------------------------------------------------
...
_G.SuaviUI_ResetReadyCheckPosition = ResetReadyCheckPosition
...
local function CreateMover()
    ...
    readyCheckMover = CreateFrame("Frame", "SuaviUI_ReadyCheckMover", UIParent, "BackdropTemplate")
...
_G.SuaviUI_ToggleReadyCheckMover = ToggleMover

---------------------------------------------------------------------------
-- HELPER FUNCTIONS
---------------------------------------------------------------------------

-- Get SUI skin colors with fallback
...
-- Create SUI-styled backdrop on a frame
...
local function SkinButton(button, sr, sg, sb, bgr, bgg, bgb, bga)
    ...
    button:HookScript("OnEnter", function(self)
        if self.suiBackdrop and self.quiHoverBg then
            self.suiBackdrop:SetBackdropColor(unpack(self.quiHoverBg))
        end
    end)
    button:HookScript("OnLeave", function(self)
        if self.suiBackdrop and self.quiNormalBg then
            self.suiBackdrop:SetBackdropColor(unpack(self.quiNormalBg))
        end
    end)
...
local function RefreshButtonColors(button, sr, sg, sb, bgr, bgg, bgb, bga)
    ...
    button.suiBackdrop:SetBackdropColor(btnBgr, btnBgg, btnBgb, bga)
    button.suiBackdrop:SetBackdropBorderColor(sr, sg, sb, 1)
end
...
local function SkinReadyCheckFrame()
    ...
    -- Create SUI backdrop on ListenerFrame (where the content is)
...
_G.SuaviUI_RefreshReadyCheckColors = RefreshReadyCheckColors
