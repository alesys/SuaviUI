...
-- SUI Color palette for popup styling
local SUI_COLORS = {
...
local function GetSettings()
    ...
    if SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.chat then
        return SUICore.db.profile.chat
    end
    return nil
end
...
local function MakeURLsClickable(text)
    ...
    local success, result = pcall(function()
        ...
        local linkFormat = "|cff" .. colorHex .. "|Haddon:suaviuichat:%1|h[%1]|h|r"
...
---------------------------------------------------------------------------
-- Create URL copy popup (on demand) - SUI styled
---------------------------------------------------------------------------
local function CreateCopyPopup()
    if urlPopup then return urlPopup end

    urlPopup = CreateFrame("Frame", "SuaviUI_ChatCopyPopup", UIParent, "BackdropTemplate")
    ...
    -- SUI color scheme
    urlPopup:SetBackdropColor(sui_COLORS.bg[1], sui_COLORS.bg[2], sui_COLORS.bg[3], sui_COLORS.bg[4])
    urlPopup:SetBackdropBorderColor(sui_COLORS.accent[1], sui_COLORS.accent[2], sui_COLORS.accent[3], sui_COLORS.accent[4])
    ...
    title:SetTextColor(sui_COLORS.accent[1], sui_COLORS.accent[2], sui_COLORS.accent[3], 1)
    ...
    editBox:SetTextColor(sui_COLORS.text[1], sui_COLORS.text[2], sui_COLORS.text[3], 1)
    ...
    if not tContains(UISpecialFrames, "SuaviUI_ChatCopyPopup") then
        tinsert(UISpecialFrames, "SuaviUI_ChatCopyPopup")
    end
...
local function SetupURLClickHandler()
    ...
    EventRegistry:RegisterCallback("SetItemRef", function(_, link, text, button)
        ...
        local url = link:match("^addon:suaviuichat:(.*)")
        if url then
            ShowCopyPopup(url)
            return true
        end
...
local function CreateChatCopyFrame()
    ...
    chatCopyFrame = CreateFrame("Frame", "SuaviUI_ChatCopyFrame", UIParent, "BackdropTemplate")
    ...
    chatCopyFrame:SetBackdropColor(sui_COLORS.bg[1], sui_COLORS.bg[2], sui_COLORS.bg[3], sui_COLORS.bg[4])
    chatCopyFrame:SetBackdropBorderColor(sui_COLORS.accent[1], sui_COLORS.accent[2], sui_COLORS.accent[3], sui_COLORS.accent[4])
    ...
    -- Add to special frames so ESC closes it
    if not tContains(UISpecialFrames, "SuaviUI_ChatCopyFrame") then
        tinsert(UISpecialFrames, "SuaviUI_ChatCopyFrame")
    end
...
local function GetOrCreateCopyButton(chatFrame)
    ...
    local button = CreateFrame("Button", frameName .. "SuaviCopyButton", chatFrame)
...
local function SetupCopyButtonHoverMode(chatFrame)
    ...
    if chatFrame.suaviCopyButtonHooked then return end
    chatFrame.suaviCopyButtonHooked = true
...
local function ApplyCopyButtonMode(chatFrame)
    ...
    elseif mode == "hover" then
        ...
        if not chatFrame.suaviCopyButtonHooked then
...
local function UpdateTabColors(tab)
    ...
    if isSelected then
        ...
        tab.__quiBackdrop:SetBackdropBorderColor(sui_COLORS.accent[1], sui_COLORS.accent[2], sui_COLORS.accent[3], 1)
    else
...
---------------------------------------------------------------------------
_G.SuaviUI_RefreshChat = RefreshAll
