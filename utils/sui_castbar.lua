--[[
    SUI Castbar Module
    Extracted from sui_unitframes.lua for better organization
...
---------------------------------------------------------------------------
local SUI_Castbar = {}
ns.SUI_Castbar = SUI_Castbar

SUI_Castbar.castbars = {}
...
-- SETUP HELPERS
---------------------------------------------------------------------------
function SUI_Castbar:SetHelpers(helpers)
    Helpers = helpers or {}
end
...
local STAGE_COLORS = SUI_Castbar.STAGE_COLORS
local STAGE_FILL_COLORS = SUI_Castbar.STAGE_FILL_COLORS
...
function SUI_Castbar:GetEmpoweredLevel()
    local playerCastbar = self.castbars["player"]
    if not playerCastbar then
        playerCastbar = _G.SuaviUI_Castbars and _G.SuaviUI_Castbars["player"]
    end
...
---------------------------------------------------------------------------
function SUI_Castbar:CreateCastbar(unitFrame, unit, unitKey)
    local settings = GetUnitSettings(unitKey)
...
---------------------------------------------------------------------------
function SUI_Castbar:SetupCastbar(castbar, unit, unitKey, castSettings)
        ...
        if spellName or channelName or isInEmpoweredHold then
            ...
            if isPlayer and self.isEmpowered then
                ...
                if self.empoweredLevelText and self.showEmpoweredLevel then
                    local currentStage, maxStages, isEmpowered = SUI_Castbar:GetEmpoweredLevel()
...
function SUI_Castbar:SetupTargetFocusCastbar(castbar, unit, unitKey, castSettings)
    self:SetupCastbar(castbar, unit, unitKey, castSettings)
end

function SUI_Castbar:SetupPlayerCastbar(castbar, unit, unitKey, castSettings)
    self:SetupCastbar(castbar, unit, unitKey, castSettings)
end
...
-- BOSS CASTBAR SETUP
---------------------------------------------------------------------------
function SUI_Castbar:SetupBossCastbar(castbar, unit, bossIndex, castSettings)
    -- Unified OnUpdate handler - handles both real casts and preview
    local function CastBar_OnUpdate(self, elapsed)
        -- Check if actually casting (real cast takes priority)
        local spellName = UnitCastingInfo(self.unit)
...
-- CREATE: Boss Castbar
---------------------------------------------------------------------------
function SUI_Castbar:CreateBossCastbar(unitFrame, unit, bossIndex)
        ...
        if spellName and startTimeMS and endTimeMS then
            ...
            C_Timer.After(0.1, function()
                if not UnitCastingInfo(self.unit) and not UnitChannelInfo(self.unit) then
                    local settings = GetUnitSettings(self.unitKey)
                    local SUI_UF = SUI_Castbar.unitFramesModule
                    local bossFramePreviewActive = SUI_UF and SUI_UF.previewMode and SUI_UF.previewMode["boss" .. self.bossIndex]
                    if settings and settings.castbar and settings.castbar.previewMode and bossFramePreviewActive then
                        -- Show preview simulation
...
-- GLOBAL FUNCTIONS
---------------------------------------------------------------------------
SUI_Castbar.unitFramesModule = nil

function SUI_Castbar:SetUnitFramesModule(ufModule)
...
_G.SuaviUI_ShowCastbarPreview = function(unitKey)
    local settings = GetUnitSettings(unitKey)
...
_G.SuaviUI_HideCastbarPreview = function(unitKey)
    local settings = GetUnitSettings(unitKey)
    if not settings or not settings.castbar then
        return
    end
...
function SUI_Castbar:RefreshCastbar(castbar, unitKey, castSettings, unitFrame)
    ...
    if newCastbar then
        ...
        if SUI_UF and SUI_UF.castbars then
            SUI_UF.castbars[unitKey] = newCastbar
        end
    end
end

function SUI_Castbar:RefreshBossCastbar(castbar, bossKey, castSettings, unitFrame)
    if not castSettings or not unitFrame then return end
    ...
    if newCastbar then
        local SUI_UF = self.unitFramesModule
        if SUI_UF and SUI_UF.castbars then
            SUI_UF.castbars[bossKey] = newCastbar
        end
    end
end

_G.SuaviUI_RefreshCastbar = function(unitKey)
    local SUI_UF = SUI_Castbar.unitFramesModule
...
_G.SuaviUI_Castbars = SUI_Castbar.castbars
