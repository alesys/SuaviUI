---------------------------------------------------------------------------
-- SuaviUI Inspect Pane Module
-- Custom inspect panel styling with equipment overlays and stats panel
-- Split from sui_character.lua for better organization
---------------------------------------------------------------------------
local ADDON_NAME, ns = ...
local SUI = ns.SUI or {}
ns.SUI = SUI
...
---------------------------------------------------------------------------
-- Import shared functions from sui_character.lua
-- These will be available after sui_character.lua loads
---------------------------------------------------------------------------
local function GetShared()
    return ns.SUI.CharacterShared or {}
end
...
local function SetupInspectTitleArea()
    ...
    if not InspectFrame._suiILvlDisplay then
        ...
        InspectFrame._suiILvlDisplay = displayFrame
    end
...
local function UpdateInspectILvlDisplay()
    if not InspectFrame or not InspectFrame._suiILvlDisplay then return end

    local settings = GetSettings()
    if settings.inspectEnabled == false then return end

    local displayFrame = InspectFrame._suiILvlDisplay
...
local function SetInspectExtendedMode(tabNum)
    ...
    if _G.SUI_InspectFrameSkinning and _G.SUI_InspectFrameSkinning.SetExtended then
        _G.SUI_InspectFrameSkinning.SetExtended(true)
    end
...
local function SetInspectNormalMode()
    ...
    if _G.SUI_InspectFrameSkinning and _G.SUI_InspectFrameSkinning.SetExtended then
        _G.SUI_InspectFrameSkinning.SetExtended(false)
    end
...
local function CreateInspectSettingsButton()
    ...
    if not (SUICore and SUICore.db and SUICore.db.profile and SUICore.db.profile.character) then
        C_Timer.After(0.5, CreateInspectSettingsButton)
        return
    end
    local charDB = SUICore.db.profile.character
    ...
    local gearBtn = CreateFrame("Button", "SUI_InspectSettingsBtn", InspectFrame, "BackdropTemplate")
    ...
    inspectSettingsPanel = CreateFrame("Frame", "SuaviUI_InspectSettingsPanel", InspectFrame, "BackdropTemplate")
    ...
    title:SetText("SUI Inspect Panel")
    ...
    if generalDB then
        bgColorPicker = GUI:CreateFormColorPicker(scrollChild, "Background Color", "skinBgColor", generalDB, function()
            ...
            if _G.SUI_InspectFrameSkinning and _G.SUI_InspectFrameSkinning.Refresh then
                _G.SUI_InspectFrameSkinning.Refresh()
            end
        end)
...
---------------------------------------------------------------------------
-- Update inspect frame (called from sui_character.lua's ScheduleUpdate)
---------------------------------------------------------------------------
...
---------------------------------------------------------------------------
-- Module API (exported for sui_character.lua to call)
---------------------------------------------------------------------------
SUI.InspectPane = {
...
ns.InspectPane = SUI.InspectPane
